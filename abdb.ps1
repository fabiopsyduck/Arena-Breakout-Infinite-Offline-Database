...
function Initialize-Database {
    ...
    $filesConfig = @{
        ...
        "DamageRange.csv"       = '"Arma";"Calibre";"nome da munição";"dano(alcance)"'
    }
...
function Initialize-CompVisuals {
    ...
    # [NOVO] O painel invisível onde o gráfico e a estação de controlo vão nascer
    $pnlDamageChart = New-Object System.Windows.Forms.Panel; $pnlDamageChart.Dock = "Fill"; $pnlDamageChart.BackColor = $theme.Background; $pnlDamageChart.Visible = $false

    # [IMPORTANTE] Salvando na variavel global
    $ui.PnlMenu = $pnlMenu
    $ui.PnlWeaponsMenu = $pnlWeaponsMenu
    $ui.PnlContent = $pnlContent
    $ui.PnlDamageChart = $pnlDamageChart
    
    $rootPanel.Controls.AddRange(@($pnlMenu, $pnlWeaponsMenu, $pnlContent, $pnlDamageChart))

    # --- 3. MENU PRINCIPAL DO MODULO ---
    $lblTitle = New-Object System.Windows.Forms.Label
    $lblTitle.Text = "Comparação e Compatibilidade"
    $lblTitle.Font = $script:fontCompTitle 
    $lblTitle.ForeColor = $theme.OrangeAccent
    $lblTitle.AutoSize = $true
    $lblTitle.Location = "50, 50"
    $pnlMenu.Controls.Add($lblTitle)

    # Botões perfeitamente alinhados e centralizados (X = 410 para ecrã de 1200px)
    $btnCompArmas = New-Object System.Windows.Forms.Button; $btnCompArmas.Text = "Comparar armas"; $btnCompArmas.Size = "380, 42"; $btnCompArmas.Location = "410, 220"
    ...
    $ui.BtnCompArmas = $btnCompArmas 

    $btnMasks = New-Object System.Windows.Forms.Button; $btnMasks.Text = "Consultar compatibilidade de máscaras e capacetes"; $btnMasks.Size = "380, 42"; $btnMasks.Location = "410, 280"
    ...
    $ui.BtnMasks = $btnMasks 

    # [NOVO BOTÃO]
    $btnDamageChart = New-Object System.Windows.Forms.Button; $btnDamageChart.Text = "Comparar Eficiência Balística"; $btnDamageChart.Size = "380, 42"; $btnDamageChart.Location = "410, 340"
    $btnDamageChart.FlatStyle = "Flat"; $btnDamageChart.BackColor = $theme.ButtonBack; $btnDamageChart.ForeColor = $theme.TextMain; $btnDamageChart.FlatAppearance.BorderColor = $theme.OrangeAccent; $btnDamageChart.FlatAppearance.MouseOverBackColor = "#555555"; $btnDamageChart.FlatAppearance.MouseDownBackColor = "#777777"
    $pnlMenu.Controls.Add($btnDamageChart)
    $ui.BtnDamageChart = $btnDamageChart
    ...
    $ui.BtnBackMain = $btnBackMain 

    # --- 4. MENU DE ARMAS ---
    $lblSub = New-Object System.Windows.Forms.Label; $lblSub.Text = "Opções de Comparação de Armas"; $lblSub.Font = $script:fontCompSub; $lblSub.ForeColor = $theme.OrangeAccent; $lblSub.AutoSize = $true; $lblSub.Location = "410, 150"
    $pnlWeaponsMenu.Controls.Add($lblSub)

    $btnBasic = New-Object System.Windows.Forms.Button; $btnBasic.Text = "Comparação Básica"; $btnBasic.Size = "380, 42"; $btnBasic.Location = "410, 250"
    ...
    $ui.BtnBasic = $btnBasic 

    $btnAdv = New-Object System.Windows.Forms.Button; $btnAdv.Text = "Comparação Avançada"; $btnAdv.Size = "380, 42"; $btnAdv.Location = "410, 310"
    ...
    $ui.BtnBackContent = $btnBackContent 
    ...
    $dgv.DefaultCellStyle.Font = $script:fontCompGridC 
...
function Register-CompEvents {
    ...
    # [NOVA NAVEGAÇÃO] O botão que chama a nova função do gráfico!
    $ui.BtnDamageChart.Add_Click({ Start-DamageChartFeature -ui $script:CompUI })
    ...
    $script:GoBackLogic = {
        ...
        elseif ($ui.PnlDamageChart.Visible) {
            # Se apertar Voltar/ESC dentro do gráfico, volta ao menu principal
...
function Start-WeaponFeature {
    ...
    foreach ($cal in $uniqueCalibers) {
        ...
        # --- PRIMEIRA LINHA: MUNIÇÕES E CALIBRE ---
        $sepIdx1 = $ui.Dgv.Rows.Add(); $sepRow1 = $ui.Dgv.Rows[$sepIdx1]
        
        # [ALTERAÇÃO APLICADA AQUI]: Removidos os parênteses
        $sepRow1.Cells[0].Value = "MUNIÇÕES: $cal"
        
        $sepRow1.DefaultCellStyle.BackColor = $theme.ButtonBack
        $sepRow1.DefaultCellStyle.ForeColor = [System.Drawing.Color]::Cyan
        $sepRow1.DefaultCellStyle.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
        $sepRow1.Height = 30
        
        # --- SEGUNDA LINHA: LISTA DE ARMAS ---
        $sepIdx2 = $ui.Dgv.Rows.Add(); $sepRow2 = $ui.Dgv.Rows[$sepIdx2]
        $sepRow2.Cells[0].Value = $wepsName
        $sepRow2.DefaultCellStyle.BackColor = $theme.ButtonBack
        $sepRow2.DefaultCellStyle.ForeColor = [System.Drawing.Color]::Cyan
        $sepRow2.DefaultCellStyle.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
        $sepRow2.DefaultCellStyle.WrapMode = [System.Windows.Forms.DataGridViewTriState]::True 
        $sepRow2.Height = 40 

        # --- CABEÇALHO DA TABELA DE MUNIÇÕES ---
...
function Start-DamageChartFeature {
    param ($ui)

    # 1. Configura Visual Base e Mostra Painel
    Set-View -ui $ui -ViewName "DamageChart"
    $pnlMain = $ui.PnlDamageChart
    
    # [CORREÇÃO MEMORY LEAK 1] Destrói ativamente os controles antigos da RAM antes de limpar o ecrã
    foreach ($ctrl in $pnlMain.Controls) {
        $ctrl.Dispose()
    }
    $pnlMain.Controls.Clear() 

    if ($null -ne $script:fastToolTip) { $script:fastToolTip.Dispose() }
    if ($null -ne $script:btnFont) { $script:btnFont.Dispose() }

    # Variáveis de Estado do Gráfico
    $script:ActiveChartCombos = New-Object System.Collections.Generic.List[Object]
    $script:ChartColors = @([System.Drawing.Color]::Orange, [System.Drawing.Color]::DodgerBlue, [System.Drawing.Color]::LimeGreen, [System.Drawing.Color]::MediumPurple, [System.Drawing.Color]::DeepPink)
    $script:ChartMarkers = @("Circle", "Square", "Triangle", "Diamond", "Cross")
    
    $script:WeaponMap = @{}
    $script:AmmoMap = @{}

    # [CORREÇÃO MEMORY LEAK 2] Cria a fonte do Botão "X" apenas UMA VEZ na memória global
    $script:btnFont = New-Object System.Drawing.Font("Segoe UI", 11, [System.Drawing.FontStyle]::Bold)

    # --- FUNÇÃO INTERNA BLINDADA: O MOTOR MATEMÁTICO LIMPO ---
    function script:Parse-DamageString {
        param([string]$rawStr)
        $pairs = $rawStr -split '\|' | ForEach-Object {
            if ($_.Trim() -match '^(\d+)\((\d+)\)$') {
                [PSCustomObject]@{ Dano = [int]$matches[1]; Alcance = [int]$matches[2]; Simulado = $false }
            }
        }
        ...
        # Simulação de queda estrita para dados isolados
        if ($plotData.Count -eq 1) {
            $danoInicial = $plotData[0].Dano
            $alcanceInicial = $plotData[0].Alcance
            $alcanceFinal = $alcanceInicial + 3
            $danoFinalBruto = $danoInicial * 0.70
            $danoFinal = [Math]::Round($danoFinalBruto, [MidpointRounding]::AwayFromZero)
            $plotData += [PSCustomObject]@{ Dano = $danoFinal; Alcance = $alcanceFinal; Simulado = $true }
        }
        
        # Converte para o formato X e Y esperado pelo Chart
        $finalPlotData = @()
        foreach ($p in $plotData) {
            $finalPlotData += [PSCustomObject]@{ X = $p.Alcance; Y = $p.Dano; Simulado = $p.Simulado }
        }
        
        return $finalPlotData
    }

    # --- 2. PREPARAR LAYOUT GERAL ---
    $pnlHeader = New-Object System.Windows.Forms.Panel; $pnlHeader.Dock = "Top"; $pnlHeader.Height = 60; $pnlHeader.BackColor = $theme.PanelBack
    $lblTitle = New-Object System.Windows.Forms.Label; $lblTitle.Text = "Comparar Eficiência Balística"; $lblTitle.Font = $script:fontCompGridH; $lblTitle.ForeColor = $theme.OrangeAccent; $lblTitle.AutoSize = $true; $lblTitle.Location = "20, 15"
    $btnBack = New-Object System.Windows.Forms.Button; $btnBack.Text = "Voltar"; $btnBack.Location = "960, 10"; $btnBack.Size = "100, 35"; $btnBack.FlatStyle = "Flat"; $btnBack.BackColor = "#333"; $btnBack.ForeColor = "White"; $btnBack.FlatAppearance.MouseOverBackColor = "#555555"; $btnBack.FlatAppearance.MouseDownBackColor = "#777777"
    
    $btnBack.Add_Click({ Set-View -ui $script:CompUI -ViewName "Menu" })
    $pnlHeader.Controls.AddRange(@($lblTitle, $btnBack))

    $pnlBottomMaster = New-Object System.Windows.Forms.Panel
    $pnlBottomMaster.Dock = "Bottom"
    $pnlBottomMaster.Height = 240 
    $pnlBottomMaster.BackColor = $theme.Background

    $pnlControls = New-Object System.Windows.Forms.Panel; $pnlControls.Dock = "Bottom"; $pnlControls.Height = 60; $pnlControls.BackColor = $theme.PanelBack
    
    $pnlTableArea = New-Object System.Windows.Forms.Panel; $pnlTableArea.Dock = "Fill"; $pnlTableArea.BackColor = $theme.Background
    $pnlTableArea.Padding = New-Object System.Windows.Forms.Padding(30, 10, 30, 10) 

    $pnlBottomMaster.Controls.Add($pnlControls)
    $pnlBottomMaster.Controls.Add($pnlTableArea)
    $pnlControls.BringToFront()

    $pnlChart = New-Object System.Windows.Forms.Panel; $pnlChart.Dock = "Fill"; $pnlChart.BackColor = $theme.Background

    $pnlMain.Controls.Add($pnlHeader)
    $pnlMain.Controls.Add($pnlBottomMaster)
    $pnlMain.Controls.Add($pnlChart)
    
    $pnlHeader.BringToFront()
    $pnlBottomMaster.BringToFront()
    $pnlChart.BringToFront()

    # --- 3. CONSTRUIR ESTAÇÃO DE CONTROLE ---
    $lblClass = New-Object System.Windows.Forms.Label; $lblClass.Text = "Classe:"; $lblClass.Location = "15, 20"; $lblClass.AutoSize = $true; $lblClass.ForeColor = $theme.TextMain
    $script:cbClass = New-Object System.Windows.Forms.ComboBox; $script:cbClass.Location = "65, 18"; $script:cbClass.Width = 100; $script:cbClass.DropDownStyle = "DropDownList"; $script:cbClass.BackColor = $theme.ButtonBack; $script:cbClass.ForeColor = $theme.TextMain
    
    $lblWeapon = New-Object System.Windows.Forms.Label; $lblWeapon.Text = "Arma:"; $lblWeapon.Location = "175, 20"; $lblWeapon.AutoSize = $true; $lblWeapon.ForeColor = $theme.TextMain
    $script:cbWeapon = New-Object System.Windows.Forms.ComboBox; $script:cbWeapon.Location = "215, 18"; $script:cbWeapon.Width = 120; $script:cbWeapon.DropDownStyle = "DropDownList"; $script:cbWeapon.BackColor = $theme.ButtonBack; $script:cbWeapon.ForeColor = $theme.TextMain; $script:cbWeapon.Enabled = $false
    
    $lblAmmo = New-Object System.Windows.Forms.Label; $lblAmmo.Text = "Munição:"; $lblAmmo.Location = "345, 20"; $lblAmmo.AutoSize = $true; $lblAmmo.ForeColor = $theme.TextMain
    $script:cbAmmo = New-Object System.Windows.Forms.ComboBox; $script:cbAmmo.Location = "405, 18"; $script:cbAmmo.Width = 120; $script:cbAmmo.DropDownStyle = "DropDownList"; $script:cbAmmo.BackColor = $theme.ButtonBack; $script:cbAmmo.ForeColor = $theme.TextMain; $script:cbAmmo.Enabled = $false

    $script:btnAdd = New-Object System.Windows.Forms.Button; $script:btnAdd.Text = "Adicionar Gráfico e Tabela"; $script:btnAdd.Location = "535, 15"; $script:btnAdd.Size = "180, 30"; $script:btnAdd.FlatStyle = "Flat"
    
    $btnReset = New-Object System.Windows.Forms.Button; $btnReset.Text = "Limpar Gráfico e Tabela"; $btnReset.Location = "725, 15"; $btnReset.Size = "165, 30"; $btnReset.BackColor = $theme.ButtonBack; $btnReset.ForeColor = $theme.TextMain; $btnReset.FlatStyle = "Flat"

    $script:lblCount = New-Object System.Windows.Forms.Label
    $script:lblCount.Text = "5 Armas restantes"
    $script:lblCount.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    $script:lblCount.Location = "895, 20"
    $script:lblCount.AutoSize = $true
    $script:lblCount.ForeColor = $theme.TextDim

    $pnlControls.Controls.AddRange(@($lblClass, $script:cbClass, $lblWeapon, $script:cbWeapon, $lblAmmo, $script:cbAmmo, $script:btnAdd, $btnReset, $script:lblCount))

    $script:dgvDmgComp = New-Object System.Windows.Forms.DataGridView
    $script:dgvDmgComp.Dock = "Fill" 
    $script:dgvDmgComp.AllowUserToAddRows = $false
    $script:dgvDmgComp.ReadOnly = $true
    $script:dgvDmgComp.RowHeadersVisible = $false
    $script:dgvDmgComp.BackgroundColor = $theme.Background
    $script:dgvDmgComp.BorderStyle = "None"
    
    $script:dgvDmgComp.DefaultCellStyle.BackColor = $theme.Background
    $script:dgvDmgComp.DefaultCellStyle.ForeColor = $theme.TextMain
    $script:dgvDmgComp.DefaultCellStyle.SelectionBackColor = $theme.Background
    $script:dgvDmgComp.DefaultCellStyle.SelectionForeColor = $theme.TextMain
    $script:dgvDmgComp.DefaultCellStyle.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter
    
    $script:dgvDmgComp.ColumnHeadersDefaultCellStyle.BackColor = $theme.ButtonBack
    $script:dgvDmgComp.ColumnHeadersDefaultCellStyle.ForeColor = $theme.OrangeAccent
    $script:dgvDmgComp.ColumnHeadersDefaultCellStyle.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    $script:dgvDmgComp.ColumnHeadersDefaultCellStyle.Alignment = [System.Windows.Forms.DataGridViewContentAlignment]::MiddleCenter
    $script:dgvDmgComp.EnableHeadersVisualStyles = $false
    $script:dgvDmgComp.AutoSizeColumnsMode = "Fill"
    $script:dgvDmgComp.ScrollBars = "Vertical"
    $script:dgvDmgComp.SelectionMode = "FullRowSelect"
    
    $script:dgvDmgComp.AllowUserToResizeColumns = $false
    $script:dgvDmgComp.AllowUserToResizeRows = $false
    $script:dgvDmgComp.AllowUserToOrderColumns = $false
    $script:dgvDmgComp.ColumnHeadersHeightSizeMode = [System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode]::DisableResizing

    $script:dgvDmgComp.ShowCellToolTips = $true

    $script:dgvDmgComp.Add_SelectionChanged({
        if ($script:dgvDmgComp.SelectedCells.Count -gt 0) {
            $script:dgvDmgComp.ClearSelection()
        }
    })

    $script:dgvDmgComp.Add_CellToolTipTextNeeded({
        param($sender, $e)
        if ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 4) {
            $e.ToolTipText = "Remover arma do gráfico"
        }
    })

    $script:dgvDmgComp.Add_CellMouseEnter({
        param($sender, $e)
        if ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 4) {
            $script:dgvDmgComp.Cursor = [System.Windows.Forms.Cursors]::Hand
        }
    })

    $script:dgvDmgComp.Add_CellMouseLeave({
        param($sender, $e)
        if ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 4) {
            $script:dgvDmgComp.Cursor = [System.Windows.Forms.Cursors]::Default
        }
    })

    $script:dgvDmgComp.Add_CellClick({
        param($sender, $e)
        if ($e.RowIndex -ge 0 -and $e.RowIndex -lt $script:ActiveChartCombos.Count -and $e.ColumnIndex -eq 4) {
            $script:ActiveChartCombos.RemoveAt($e.RowIndex)
            script:Update-ChartEngine
        }
    })

    $script:dgvDmgComp.Add_CellPainting({
        param($sender, $e)
        if ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 1 -and $e.Value) {
            $e.Paint($e.CellBounds, [System.Windows.Forms.DataGridViewPaintParts]::Background -bor [System.Windows.Forms.DataGridViewPaintParts]::Border -bor [System.Windows.Forms.DataGridViewPaintParts]::SelectionBackground -bor [System.Windows.Forms.DataGridViewPaintParts]::Focus)
            
            $txt = $e.Value.ToString()
            $separator = " / "
            $idxSplit = $txt.IndexOf($separator)
            
            if ($idxSplit -ge 0) {
                $leftTxt = $txt.Substring(0, $idxSplit)
                $rightTxt = $txt.Substring($idxSplit + $separator.Length)
                
                $valLeft = 0; $valRight = 0
                if ($leftTxt -match '^(\d+)') { $valLeft = [int]$matches[1] }
                if ($rightTxt -match '^(\d+)') { $valRight = [int]$matches[1] }
                
                $rightColor = $theme.TextMain
                if ($valLeft -gt 0) {
                    if ($valRight -gt $valLeft) { $rightColor = [System.Drawing.Color]::LimeGreen }
                    elseif ($valRight -lt $valLeft) { $rightColor = [System.Drawing.Color]::IndianRed }
                }
                ...
                [int]$wLeft = [System.Windows.Forms.TextRenderer]::MeasureText($e.Graphics, $leftTxt, $font, [System.Drawing.Size]::Empty, $flags).Width
                [int]$wSep = [System.Windows.Forms.TextRenderer]::MeasureText($e.Graphics, $separator, $font, [System.Drawing.Size]::Empty, $flags).Width
                [int]$wRight = [System.Windows.Forms.TextRenderer]::MeasureText($e.Graphics, $rightTxt, $font, [System.Drawing.Size]::Empty, $flags).Width
                
                [int]$totalWidth = $wLeft + $wSep + $wRight
                [int]$startX = $e.CellBounds.X + [Math]::Round(($e.CellBounds.Width - $totalWidth) / 2)
                
                $rectLeft = New-Object System.Drawing.Rectangle($startX, $e.CellBounds.Y, $wLeft, $e.CellBounds.Height)
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $leftTxt, $font, $rectLeft, $theme.TextMain, $flags)
                
                [int]$sepX = $startX + $wLeft
                $rectSep = New-Object System.Drawing.Rectangle($sepX, $e.CellBounds.Y, $wSep, $e.CellBounds.Height)
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $separator, $font, $rectSep, $theme.TextDim, $flags)

                [int]$rightX = $sepX + $wSep
                $rectRight = New-Object System.Drawing.Rectangle($rightX, $e.CellBounds.Y, $wRight, $e.CellBounds.Height)
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $rightTxt, $font, $rectRight, $rightColor, $flags)
            } else {
                $font = $e.CellStyle.Font
                $flags = [System.Windows.Forms.TextFormatFlags]::HorizontalCenter -bor [System.Windows.Forms.TextFormatFlags]::VerticalCenter
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $txt, $font, $e.CellBounds, $theme.TextMain, $flags)
            }
            $e.Handled = $true
        }
        elseif ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 2 -and $e.Value) {
            $e.Paint($e.CellBounds, [System.Windows.Forms.DataGridViewPaintParts]::Background -bor [System.Windows.Forms.DataGridViewPaintParts]::Border -bor [System.Windows.Forms.DataGridViewPaintParts]::SelectionBackground -bor [System.Windows.Forms.DataGridViewPaintParts]::Focus)
            ...
            if ($idxSplit -ge 0 -and -not $txt.Contains("Fixo/Padrão")) {
                $leftTxt = $txt.Substring(0, $idxSplit)
                $rightTxt = $txt.Substring($idxSplit + $separator.Length)
                ...
                if ($txt.Contains("Fixo/Padrão")) { $color = $theme.TextDim }
                
                $flags = [System.Windows.Forms.TextFormatFlags]::HorizontalCenter -bor [System.Windows.Forms.TextFormatFlags]::VerticalCenter
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $txt, $font, $e.CellBounds, $color, $flags)
            }
            $e.Handled = $true
        }
        elseif ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 3 -and $e.Value) {
            $e.Paint($e.CellBounds, [System.Windows.Forms.DataGridViewPaintParts]::Background -bor [System.Windows.Forms.DataGridViewPaintParts]::Border -bor [System.Windows.Forms.DataGridViewPaintParts]::SelectionBackground -bor [System.Windows.Forms.DataGridViewPaintParts]::Focus)
            ...
            if ($idxSplit -ge 0) {
                $dmgTxt = $txt.Substring(0, $idxSplit)
                $sepTxt = $separator
                $rangeTxt = $txt.Substring($idxSplit + $separator.Length)
            } else {
                $dmgTxt = $txt
            }
            
            $dmgColor = $theme.TextMain
            if ($dmgTxt.StartsWith("+")) { $dmgColor = [System.Drawing.Color]::LimeGreen }
            elseif ($dmgTxt.StartsWith("-")) { $dmgColor = [System.Drawing.Color]::IndianRed }
            
            $sepColor = $theme.TextDim

            $rangeColor = $theme.TextMain
            if ($rangeTxt.StartsWith("+")) { $rangeColor = [System.Drawing.Color]::LimeGreen }
            elseif ($rangeTxt.StartsWith("-")) { $rangeColor = [System.Drawing.Color]::IndianRed }
            
            $font = $e.CellStyle.Font
            $flags = [System.Windows.Forms.TextFormatFlags]::NoPadding -bor [System.Windows.Forms.TextFormatFlags]::VerticalCenter
            
            [int]$wDmg = [System.Windows.Forms.TextRenderer]::MeasureText($e.Graphics, $dmgTxt, $font, [System.Drawing.Size]::Empty, $flags).Width
            
            [int]$wSep = 0
            if ($sepTxt -ne "") {
                $wSep = [System.Windows.Forms.TextRenderer]::MeasureText($e.Graphics, $sepTxt, $font, [System.Drawing.Size]::Empty, $flags).Width
            }

            [int]$wRange = 0
            if ($rangeTxt -ne "") {
                $wRange = [System.Windows.Forms.TextRenderer]::MeasureText($e.Graphics, $rangeTxt, $font, [System.Drawing.Size]::Empty, $flags).Width
            }
            
            [int]$totalWidth = $wDmg + $wSep + $wRange
            [int]$startX = $e.CellBounds.X + [Math]::Round(($e.CellBounds.Width - $totalWidth) / 2)
            
            $rectDmg = New-Object System.Drawing.Rectangle($startX, $e.CellBounds.Y, $wDmg, $e.CellBounds.Height)
            [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $dmgTxt, $font, $rectDmg, $dmgColor, $flags)
            
            if ($sepTxt -ne "") {
                [int]$sepX = $startX + $wDmg
                $rectSep = New-Object System.Drawing.Rectangle($sepX, $e.CellBounds.Y, $wSep, $e.CellBounds.Height)
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $sepTxt, $font, $rectSep, $sepColor, $flags)
            }

            if ($rangeTxt -ne "") {
                [int]$rangeX = $startX + $wDmg + $wSep
                $rectRange = New-Object System.Drawing.Rectangle($rangeX, $e.CellBounds.Y, $wRange, $e.CellBounds.Height)
                [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $rangeTxt, $font, $rectRange, $rangeColor, $flags)
            }
            
            $e.Handled = $true
        }
        elseif ($e.RowIndex -ge 0 -and $e.ColumnIndex -eq 4 -and $e.Value) {
            $e.Paint($e.CellBounds, [System.Windows.Forms.DataGridViewPaintParts]::Background -bor [System.Windows.Forms.DataGridViewPaintParts]::Border -bor [System.Windows.Forms.DataGridViewPaintParts]::SelectionBackground -bor [System.Windows.Forms.DataGridViewPaintParts]::Focus)
            ...
            $flags = [System.Windows.Forms.TextFormatFlags]::HorizontalCenter -bor [System.Windows.Forms.TextFormatFlags]::VerticalCenter
            [System.Windows.Forms.TextRenderer]::DrawText($e.Graphics, $txt, $script:btnFont, $e.CellBounds, $color, $flags)
            
            $e.Handled = $true
        }
    })

    $pnlTableArea.Controls.Add($script:dgvDmgComp)

    # --- MOTOR INTELIGENTE DE MUNIÇÕES ---
    function script:Update-AmmoDropdown {
        $script:cbAmmo.Items.Clear()
        $script:AmmoMap.Clear()
        $script:cbAmmo.Enabled = $false
        
        if (-not $script:cbWeapon.SelectedItem -or $script:cbWeapon.SelectedItem -eq "-- Selecione --") {
            script:Validate-AddButton 
            return
        }

        $wReal = $script:WeaponMap[$script:cbWeapon.SelectedItem]
        
        $calibreArma = ""
        $dmgRow = $script:damageData | Where-Object { $_.Arma -eq $wReal } | Select-Object -First 1
        if ($dmgRow -and $null -ne $dmgRow.Calibre -and $dmgRow.Calibre -ne "") {
            $calibreArma = $dmgRow.Calibre.Trim()
        } else {
            $wInfo = $script:availableWeapons | Where-Object { $_.Nome -eq $wReal } | Select-Object -First 1
            if ($wInfo -and $null -ne $wInfo.Calibre -and $wInfo.Calibre -ne "") {
                $calibreArma = $wInfo.Calibre.Trim()
            }
        }

        $ammos = $script:damageData | Where-Object { $_.Arma -eq $wReal } | Select-Object -ExpandProperty "nome da munição" | Select-Object -Unique
        
        $sortedAmmos = $ammos | ForEach-Object {
            $aName = $_
            
            $isInUse = $false
            foreach ($c in $script:ActiveChartCombos) {
                if ($c.Weapon -eq $wReal -and $c.Ammo -eq $aName) {
                    $isInUse = $true
                    break
                }
            }
            ...
            if (-not $isInUse) {
                $ammoRow = $null
                if ($calibreArma -ne "") {
                    $ammoRow = $script:allAmmoData | Where-Object { $_.NomeItem -eq $aName -and $null -ne $_.Calibre -and $_.Calibre.Trim() -eq $calibreArma } | Select-Object -First 1
                }
                
                if (-not $ammoRow) {
                    $ammoRow = $script:allAmmoData | Where-Object { $_.NomeItem -eq $aName } | Select-Object -First 1
                }
                
                $lvlNum = 99 
                $nivelDisplay = "?"
                
                if ($ammoRow -and $null -ne $ammoRow.NivelPenetracao -and $ammoRow.NivelPenetracao -ne "") {
                    $rawLvl = $ammoRow.NivelPenetracao.Trim()
                    if ($rawLvl -match '^\d+$') { 
                        $lvlNum = [int]$rawLvl
                        $nivelDisplay = "Lv.$rawLvl" 
                    } else {
                        $nivelDisplay = $rawLvl
                    }
                }
                ...
                [PSCustomObject]@{
                    Name = $aName
                    LvlNum = $lvlNum
                    Display = "$aName ($nivelDisplay)"
                }
            }
        } | Sort-Object LvlNum, Name
        
        $script:cbAmmo.Items.Add("-- Selecione --") | Out-Null
        if ($sortedAmmos) {
            foreach ($aObj in $sortedAmmos) {
                $script:AmmoMap[$aObj.Display] = $aObj.Name
                $script:cbAmmo.Items.Add($aObj.Display) | Out-Null
            }
            $script:cbAmmo.Enabled = $true
        }
        $script:cbAmmo.SelectedIndex = 0
        
        script:Validate-AddButton 
    }

    function script:Validate-AddButton {
        $wDisp = $script:cbWeapon.SelectedItem
        $aDisp = $script:cbAmmo.SelectedItem
        
        $dimButton = {
            param($msg)
            $script:btnAdd.Text = $msg
            $script:btnAdd.BackColor = [System.Drawing.Color]::FromArgb(35, 35, 38)
            $script:btnAdd.ForeColor = [System.Drawing.Color]::FromArgb(100, 100, 100)
            $script:btnAdd.FlatAppearance.BorderColor = [System.Drawing.Color]::FromArgb(50, 50, 50)
            $script:btnAdd.Cursor = [System.Windows.Forms.Cursors]::No
        }

        $lightButton = {
            $script:btnAdd.Text = "Adicionar Gráfico e Tabela"
            $script:btnAdd.BackColor = $theme.OrangeAccent
            $script:btnAdd.ForeColor = $theme.Background
            $script:btnAdd.FlatAppearance.BorderColor = $theme.OrangeAccent
            $script:btnAdd.Cursor = [System.Windows.Forms.Cursors]::Hand
        }

        if ($script:ActiveChartCombos.Count -ge 5) { &$dimButton "Limite Atingido (5/5)"; return }
        if (-not $wDisp -or $wDisp -eq "-- Selecione --" -or -not $aDisp -or $aDisp -eq "-- Selecione --") { &$dimButton "Adicionar Gráfico e Tabela"; return }

        &$lightButton
    }

    # --- 4. CARREGAMENTO DE DADOS OFFLINE E POPULAÇÃO DOS DROPDOWNS ---
    $script:availableWeapons = Get-WeaponData
    
    $dbPath = Get-DatabasePath
    $damageCsvPath = Join-Path $dbPath "DamageRange.csv"
    $ammoCsvPath = Join-Path $dbPath "Ammo.csv"
    
    $script:damageData = if (Test-Path $damageCsvPath) { Import-Csv $damageCsvPath -Delimiter ";" -Encoding UTF8 } else { @() }
    $script:allAmmoData = if (Test-Path $ammoCsvPath) { Import-Csv $ammoCsvPath -Delimiter ";" -Encoding UTF8 } else { @() }
    
    $weaponsWithData = $script:damageData | Select-Object -ExpandProperty Arma -Unique
    $script:filteredWeps = $script:availableWeapons | Where-Object { $weaponsWithData -contains $_.Nome }
    
    $classes = $script:filteredWeps | Select-Object -ExpandProperty ClasseDisplay -Unique | Sort-Object
    $script:cbClass.Items.Add("-- Selecione --") | Out-Null
    foreach ($c in $classes) { $script:cbClass.Items.Add($c) | Out-Null }
    $script:cbClass.SelectedIndex = 0

    $script:cbClass.Add_SelectedIndexChanged({
        $script:cbWeapon.Items.Clear(); $script:cbAmmo.Items.Clear()
        $script:WeaponMap.Clear(); $script:AmmoMap.Clear()
        $script:cbWeapon.Enabled = $false; $script:cbAmmo.Enabled = $false
        
        if ($script:cbClass.SelectedItem -ne "-- Selecione --") {
            $wepsInClass = $script:filteredWeps | Where-Object { $_.ClasseDisplay -eq $script:cbClass.SelectedItem } | Sort-Object Nome
            $script:cbWeapon.Items.Add("-- Selecione --") | Out-Null
            
            foreach ($w in $wepsInClass) {
                $cal = if ($w.Calibre) { $w.Calibre } else { "N/A" }
                $displayText = "$($w.Nome) ($cal)"
                $script:WeaponMap[$displayText] = $w.Nome
                $script:cbWeapon.Items.Add($displayText) | Out-Null
            }
            $script:cbWeapon.Enabled = $true; $script:cbWeapon.SelectedIndex = 0
        }
        script:Validate-AddButton 
    })
    
    $script:cbWeapon.Add_SelectedIndexChanged({
        script:Update-AmmoDropdown
    })

    $script:cbAmmo.Add_SelectedIndexChanged({ script:Validate-AddButton })

    # --- 5. CONSTRUÇÃO DO GRÁFICO ---
    $script:dmgChart = New-Object System.Windows.Forms.DataVisualization.Charting.Chart
    $script:dmgChart.Dock = "Fill"; $script:dmgChart.BackColor = $theme.Background
    
    $chartArea = New-Object System.Windows.Forms.DataVisualization.Charting.ChartArea; $chartArea.Name = "MainArea"; $chartArea.BackColor = $theme.PanelBack

    $chartArea.Position.Auto = $false; $chartArea.Position.X = 2; $chartArea.Position.Y = 8; $chartArea.Position.Width = 96; $chartArea.Position.Height = 92
    $chartArea.InnerPlotPosition.Auto = $false; $chartArea.InnerPlotPosition.X = 8; $chartArea.InnerPlotPosition.Y = 2; $chartArea.InnerPlotPosition.Width = 90; $chartArea.InnerPlotPosition.Height = 82
    ...
    $chartArea.AxisX.ScaleView.Zoomable = $true; $chartArea.AxisX.ScrollBar.IsPositionedInside = $true
    $chartArea.AxisY.ScaleView.Zoomable = $true; $chartArea.AxisY.ScrollBar.IsPositionedInside = $true

    $chartArea.AxisX.ScrollBar.BackColor = [System.Drawing.Color]::FromArgb(45, 45, 48)
    $chartArea.AxisX.ScrollBar.ButtonColor = [System.Drawing.Color]::FromArgb(140, 140, 140) 
    $chartArea.AxisX.ScrollBar.LineColor = [System.Drawing.Color]::FromArgb(70, 70, 70)
    $chartArea.AxisY.ScrollBar.BackColor = [System.Drawing.Color]::FromArgb(45, 45, 48)
    $chartArea.AxisY.ScrollBar.ButtonColor = [System.Drawing.Color]::FromArgb(140, 140, 140)
    $chartArea.AxisY.ScrollBar.LineColor = [System.Drawing.Color]::FromArgb(70, 70, 70)

    $chartArea.AxisX.Title = "Distância (Metros)"; $chartArea.AxisX.TitleFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold); $chartArea.AxisX.TitleForeColor = $theme.OrangeAccent; $chartArea.AxisX.LabelStyle.ForeColor = $theme.TextMain; $chartArea.AxisX.LabelStyle.Font = New-Object System.Drawing.Font("Segoe UI", 9); $chartArea.AxisX.LineColor = $theme.TextDim; $chartArea.AxisX.MajorGrid.LineColor = [System.Drawing.Color]::FromArgb(70, 70, 70)
    $chartArea.AxisY.Title = "Dano Total"; $chartArea.AxisY.TitleFont = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold); $chartArea.AxisY.TitleForeColor = $theme.OrangeAccent; $chartArea.AxisY.LabelStyle.ForeColor = $theme.TextMain; $chartArea.AxisY.LabelStyle.Font = New-Object System.Drawing.Font("Segoe UI", 9); $chartArea.AxisY.LineColor = $theme.TextDim; $chartArea.AxisY.MajorGrid.LineColor = [System.Drawing.Color]::FromArgb(70, 70, 70)

    $chartArea.AxisX.IsLabelAutoFit = $false; $chartArea.AxisX.LabelStyle.IsStaggered = $false; $chartArea.AxisX.LabelStyle.Format = "0"
    $chartArea.AxisY.IsLabelAutoFit = $false; $chartArea.AxisY.LabelStyle.IsStaggered = $false; $chartArea.AxisY.LabelStyle.Format = "0"
    $script:dmgChart.ChartAreas.Add($chartArea)

    $legend = New-Object System.Windows.Forms.DataVisualization.Charting.Legend; $legend.Name = "Default"; $legend.BackColor = [System.Drawing.Color]::Transparent; $legend.ForeColor = $theme.TextMain; $legend.Docking = "Top"; $legend.Alignment = "Center"
    $script:dmgChart.Legends.Add($legend)
    $pnlChart.Controls.Add($script:dmgChart)

    # --- 6. EVENTOS DE INTERAÇÃO DO GRÁFICO E RADAR ---
    $script:dmgChart.Add_MouseEnter({ param($sender, $e) $sender.Focus() })
    
    $script:dmgChart.Add_MouseWheel({
        param($sender, $e)
        try {
            $ca = $sender.ChartAreas["MainArea"]
            $ax = $ca.AxisX; $ay = $ca.AxisY
            
            $xVal = $ax.PixelPositionToValue($e.X); $yVal = $ay.PixelPositionToValue($e.Y)
            $zoomFactor = 0.25
            
            $xMin = $ax.ScaleView.ViewMinimum; if ([double]::IsNaN($xMin)) { $xMin = $ax.Minimum }
            $xMax = $ax.ScaleView.ViewMaximum; if ([double]::IsNaN($xMax)) { $xMax = $ax.Maximum }
            $xRange = $xMax - $xMin
            
            $yMin = $ay.ScaleView.ViewMinimum; if ([double]::IsNaN($yMin)) { $yMin = $ay.Minimum }
            $yMax = $ay.ScaleView.ViewMaximum; if ([double]::IsNaN($yMax)) { $yMax = $ay.Maximum }
            $yRange = $yMax - $yMin
            
            if ($e.Delta -gt 0) { $newXRange = $xRange * (1 - $zoomFactor); $newYRange = $yRange * (1 - $zoomFactor) } 
            else { $newXRange = $xRange * (1 + $zoomFactor); $newYRange = $yRange * (1 + $zoomFactor) }
            
            $xInt = if ($ax.Interval -gt 0) { $ax.Interval } else { 1 }
            $yInt = if ($ay.Interval -gt 0) { $ay.Interval } else { 1 }
            if ($newXRange -lt ($xInt * 4)) { $newXRange = $xInt * 4 }
            if ($newYRange -lt ($yInt * 4)) { $newYRange = $yInt * 4 }
            
            $rawXMin = $xVal - ($newXRange * (($xVal - $xMin) / $xRange)); $rawXMax = $rawXMin + $newXRange
            $rawYMin = $yVal - ($newYRange * (($yVal - $yMin) / $yRange)); $rawYMax = $rawYMin + $newYRange
            
            $newXMin = [Math]::Floor($rawXMin / $xInt) * $xInt; $newXMax = [Math]::Ceiling($rawXMax / $xInt) * $xInt
            $newYMin = [Math]::Floor($rawYMin / $yInt) * $yInt; $newYMax = [Math]::Ceiling($rawYMax / $yInt) * $yInt
            
            if (($newXMax - $newXMin) -ge ($ax.Maximum - $ax.Minimum)) { $ax.ScaleView.ZoomReset(0) } else { $ax.ScaleView.Zoom($newXMin, $newXMax) }
            if (($newYMax - $newYMin) -ge ($ay.Maximum - $ay.Minimum)) { $ay.ScaleView.ZoomReset(0) } else { $ay.ScaleView.Zoom($newYMin, $newYMax) }
        } catch {}
    })

    $script:fastToolTip = New-Object System.Windows.Forms.ToolTip
    $script:fastToolTip.UseAnimation = $false; $script:fastToolTip.UseFading = $false
    $script:simulatedPoints = @{}
    $script:lastHitKey = $null

    $script:dmgChart.Add_MouseMove({
        param($sender, $e)
        $hit = $sender.HitTest($e.X, $e.Y)
        
        if ($hit.ChartElementType -eq [System.Windows.Forms.DataVisualization.Charting.ChartElementType]::DataPoint) {
            $idxHit = $hit.PointIndex
            $ptsHit = $hit.Series.Points
            
            if ($idxHit -eq 0 -or $idxHit -eq ($ptsHit.Count - 1)) { 
                $script:fastToolTip.Hide($sender); $script:lastHitKey = $null; return 
            }
            
            $dpHit = $ptsHit[$idxHit]
            $targetX = $dpHit.XValue
            $targetY = $dpHit.YValues[0]
            
            try {
                $ca = $sender.ChartAreas[0]
                $pixelX = $ca.AxisX.ValueToPixelPosition($targetX)
                $pixelY = $ca.AxisY.ValueToPixelPosition($targetY)
                
                if ([Math]::Abs($e.X - $pixelX) -le 8 -and [Math]::Abs($e.Y - $pixelY) -le 8) {
                    $hitKey = "$targetX-$targetY"
                    
                    if ($script:lastHitKey -ne $hitKey) {
                        $script:lastHitKey = $hitKey
                        $nl = [Environment]::NewLine
                        
                        $distanciaReal = [Math]::Truncate($targetX)
                        $tooltipTxt = "=== Distância: ${distanciaReal}m ===$nl"
                        $matchesList = @()
                        ...
                        foreach ($s in $sender.Series) {
                            ...
                            for ($i = 1; $i -lt ($pts.Count - 1); $i++) {
                                $p = $pts[$i]
                                if ([Math]::Abs($p.XValue - $targetX) -lt 0.01 -and $p.YValues[0] -eq $targetY) {
                                    $nomeCombo = $s.Name
                                    
                                    # LÓGICA DE DADOS LIMPA (Apenas Inicial e Final)
                                    if ($i -eq 1) { $tipo = "Inicial" }
                                    else { 
                                        if ($script:simulatedPoints["$nomeCombo-$i"]) { $tipo = "Final Simulado" } else { $tipo = "Final" }
                                    }
                                    
                                    $danoTotal = $p.YValues[0]
                                    $comboInfo = $script:ActiveChartCombos | Where-Object { $_.Name -eq $nomeCombo } | Select-Object -First 1
                                    $danoDisplay = "$danoTotal"
                                    
                                    if ($comboInfo -and $comboInfo.PelletCount -gt 1) {
                                        $danoPorPellet = [Math]::Round($danoTotal / $comboInfo.PelletCount)
                                        $danoDisplay = "$danoPorPellet$($comboInfo.PelletText) ($danoTotal)"
                                    }

                                    $matchesList += "[$nomeCombo]$nl" + "Tipo: $tipo$nl" + "Dano: $danoDisplay"
                                }
                            }
                        }
                        
                        $finalTxt = $tooltipTxt + ($matchesList -join "$nl$nl")
                        $script:fastToolTip.Show($finalTxt, $sender, $e.X + 15, $e.Y + 15)
                    }
                } else {
                    $script:fastToolTip.Hide($sender); $script:lastHitKey = $null
                }
            } catch {
                $script:fastToolTip.Hide($sender); $script:lastHitKey = $null
            }
    ...
    # --- 7. ATUALIZAÇÃO DO GRÁFICO E DA TABELA ---
    function script:Update-ChartEngine {
        $script:dmgChart.Series.Clear()
        $script:dmgChart.ChartAreas["MainArea"].AxisX.ScaleView.ZoomReset(0)
        $script:dmgChart.ChartAreas["MainArea"].AxisY.ScaleView.ZoomReset(0)
        $script:lastHitKey = $null; $script:simulatedPoints.Clear()

        for($i=0; $i -lt $script:ActiveChartCombos.Count; $i++) {
            $script:ActiveChartCombos[$i].Color = $script:ChartColors[$i]
            $script:ActiveChartCombos[$i].Marker = $script:ChartMarkers[$i]
        }
        
        $restantes = 5 - $script:ActiveChartCombos.Count
        $script:lblCount.Text = "$restantes Armas restantes"

        if ($script:dgvDmgComp.DataSource -is [System.Data.DataTable]) { $script:dgvDmgComp.DataSource.Dispose() }
        
        $dtInfo = New-Object System.Data.DataTable
        
        $dtInfo.Columns.Add("Combo (Arma + Munição)") | Out-Null
        $dtInfo.Columns.Add("Dano (Munição / Arma)") | Out-Null
        $dtInfo.Columns.Add("Alcance (Base / Efetividade Máxima)") | Out-Null
        $dtInfo.Columns.Add("Eficiência Balística") | Out-Null
        $dtInfo.Columns.Add("Remover") | Out-Null 

        $gMinX = 9999; $gMaxX = 0; $gMinY = 9999; $gMaxY = 0

        foreach ($combo in $script:ActiveChartCombos) {
            if ($combo.Data.Count -gt 0) {
                if ($combo.Data[0].X -lt $gMinX) { $gMinX = $combo.Data[0].X }
                if ($combo.Data[-1].X -gt $gMaxX) { $gMaxX = $combo.Data[-1].X }
                foreach ($p in $combo.Data) {
                    if ($p.Y -lt $gMinY) { $gMinY = $p.Y }
                    if ($p.Y -gt $gMaxY) { $gMaxY = $p.Y }
                }
            }

            $rowInfo = $dtInfo.NewRow()
            
            $rowInfo[0] = $combo.Name
            
            $wepMaxTotal = 0
            if ($combo.Data.Count -gt 0) { $wepMaxTotal = ($combo.Data | Measure-Object -Property Y -Maximum).Maximum }
            $ammoBaseDmg = $combo.BaseDmg
            $ammoTotalBaseDmg = $ammoBaseDmg * $combo.PelletCount

            if ($combo.PelletCount -gt 1) {
                $wepPorPellet = [Math]::Round($wepMaxTotal / $combo.PelletCount)
                $rowInfo[1] = "$ammoBaseDmg$($combo.PelletText) / $wepPorPellet$($combo.PelletText)"
            } else {
                $ammoStr = if ($ammoBaseDmg -gt 0) { "$ammoBaseDmg" } else { "Desc." }
                $rowInfo[1] = "$ammoStr / $wepMaxTotal"
            }
            
            $wInfo = $script:availableWeapons | Where-Object { $_.Nome -eq $combo.Weapon } | Select-Object -First 1
            $baseRange = 0
            if ($wInfo -and $wInfo.Alcance -match '(\d+)') {
                $baseRange = [int]$Matches[1]
            }
            
            $effRange = 0
            if ($combo.Data.Count -gt 0) { $effRange = $combo.Data[0].X }
            
            $rangeGain = $effRange - $baseRange

            if ($baseRange -eq 0) {
                $rowInfo[2] = "? / $($effRange)m"
            } elseif ($baseRange -eq $effRange) {
                $rowInfo[2] = "$($baseRange)m (Fixo/Padrão)"
            } else {
                $rowInfo[2] = "$($baseRange)m / $($effRange)m"
            }

            if ($ammoBaseDmg -gt 0) {
                $bonusDmg = [Math]::Round((($wepMaxTotal - $ammoTotalBaseDmg) / $ammoTotalBaseDmg) * 100, 1)
                
                $effParts = @()
                
                if ($bonusDmg -ne 0) {
                    $signDmg = if ($bonusDmg -gt 0) { "+" } else { "" }
                    $effParts += "$signDmg$bonusDmg% Dano"
                }
                
                if ($rangeGain -ne 0) {
                    $signRange = if ($rangeGain -gt 0) { "+" } else { "" }
                    $effParts += "$signRange$rangeGain`m Alcance"
                }
                
                if ($effParts.Count -eq 2) {
                    $rowInfo[3] = $effParts -join "  |  "
                } elseif ($effParts.Count -eq 1) {
                    $rowInfo[3] = $effParts[0]
                } else {
                    $rowInfo[3] = "Padrão"
                }
            } else {
        ...
        $script:dgvDmgComp.DataSource = $dtInfo
        
        if ($script:dgvDmgComp.Columns["Remover"]) {
            $script:dgvDmgComp.Columns["Remover"].HeaderText = ""
        }

        foreach ($col in $script:dgvDmgComp.Columns) {
            $col.SortMode = [System.Windows.Forms.DataGridViewColumnSortMode]::NotSortable
        }

        if ($script:dgvDmgComp.Columns.Count -gt 4) {
            $script:dgvDmgComp.Columns[4].AutoSizeMode = [System.Windows.Forms.DataGridViewAutoSizeColumnMode]::None
            $script:dgvDmgComp.Columns[4].Width = 40
        }

        if ($gMinX -eq 9999) { $gMinX = 0; $gMaxX = 20; $gMinY = 0; $gMaxY = 100 }
        $axisMinX = $gMinX - 2; if ($axisMinX -lt 0) { $axisMinX = 0 }
        $axisMaxX = $gMaxX + 2
        
        $script:dmgChart.ChartAreas["MainArea"].AxisX.Minimum = [double]::NaN
        $script:dmgChart.ChartAreas["MainArea"].AxisX.Maximum = [double]::NaN
        $script:dmgChart.ChartAreas["MainArea"].AxisY.Minimum = [double]::NaN
        $script:dmgChart.ChartAreas["MainArea"].AxisY.Maximum = [double]::NaN

        $axisMinY = [Math]::Floor($gMinY / 5) * 5 - 5; if ($axisMinY -lt 0) { $axisMinY = 0 }
        $axisMaxY = [Math]::Ceiling($gMaxY / 5) * 5 + 5

        $script:dmgChart.ChartAreas["MainArea"].AxisX.Minimum = $axisMinX; $script:dmgChart.ChartAreas["MainArea"].AxisX.Maximum = $axisMaxX
        $script:dmgChart.ChartAreas["MainArea"].AxisY.Minimum = $axisMinY; $script:dmgChart.ChartAreas["MainArea"].AxisY.Maximum = $axisMaxY

        $yRange = $axisMaxY - $axisMinY
        if ($yRange -gt 300) { $script:dmgChart.ChartAreas["MainArea"].AxisY.Interval = 50 } 
        elseif ($yRange -gt 150) { $script:dmgChart.ChartAreas["MainArea"].AxisY.Interval = 20 } 
        elseif ($yRange -gt 80) { $script:dmgChart.ChartAreas["MainArea"].AxisY.Interval = 10 } 
        elseif ($yRange -gt 40) { $script:dmgChart.ChartAreas["MainArea"].AxisY.Interval = 5 } 
        else { $script:dmgChart.ChartAreas["MainArea"].AxisY.Interval = 1 }

        $xRange = $axisMaxX - $axisMinX
        if ($xRange -gt 100) { $script:dmgChart.ChartAreas["MainArea"].AxisX.Interval = 10 } elseif ($xRange -gt 50) { $script:dmgChart.ChartAreas["MainArea"].AxisX.Interval = 5 } elseif ($xRange -gt 25) { $script:dmgChart.ChartAreas["MainArea"].AxisX.Interval = 2 } else { $script:dmgChart.ChartAreas["MainArea"].AxisX.Interval = 1 }
        
        $script:dmgChart.ChartAreas["MainArea"].AxisX.LabelStyle.Angle = -90 

        foreach ($combo in $script:ActiveChartCombos) {
            $s = New-Object System.Windows.Forms.DataVisualization.Charting.Series($combo.Name)
            $s.ChartType = "Line" 
            $s.BorderWidth = 3
            $s.Color = $combo.Color
            $s.MarkerStyle = $combo.Marker; $s.MarkerSize = 8; $s.MarkerColor = [System.Drawing.Color]::White
            
            $idxStart = $s.Points.AddXY($axisMinX, $combo.Data[0].Y)
            $s.Points[$idxStart].MarkerStyle = "None"
            
            foreach ($dp in $combo.Data) { 
                $realIdx = $s.Points.AddXY($dp.X, $dp.Y)
                if ($dp.Simulado) { $script:simulatedPoints["$($combo.Name)-$realIdx"] = $true }
            }
            
            $idxEnd = $s.Points.AddXY($axisMaxX, $combo.Data[-1].Y)
            $s.Points[$idxEnd].MarkerStyle = "None"
            $script:dmgChart.Series.Add($s)
        }
        
        script:Update-AmmoDropdown
    }

    # --- 8. LIGAÇÃO DOS BOTÕES ---
    $script:btnAdd.Add_Click({
        if ($script:btnAdd.Text -ne "Adicionar Gráfico e Tabela") { return }
        
        $wDisp = $script:cbWeapon.SelectedItem; $aDisp = $script:cbAmmo.SelectedItem
        $wReal = $script:WeaponMap[$wDisp]
        $aReal = $script:AmmoMap[$aDisp]
        
        $aDispFormatted = $aDisp -replace ' \(([^)]+)\)$', ' - $1'
        $comboName = "$wReal ($aDispFormatted)"
        
        $row = $script:damageData | Where-Object { $_.Arma -eq $wReal -and $_."nome da munição" -eq $aReal } | Select-Object -First 1
        if (-not $row) { return }
        
        $parsed = script:Parse-DamageString -rawStr $row."dano(alcance)"
        ...
        $ammoRow = $null
        if ($calibreArma -ne "") {
            $ammoRow = $script:allAmmoData | Where-Object { $_.NomeItem -eq $aReal -and $null -ne $_.Calibre -and $_.Calibre.Trim() -eq $calibreArma } | Select-Object -First 1
        }
        if (-not $ammoRow) {
            $ammoRow = $script:allAmmoData | Where-Object { $_.NomeItem -eq $aReal } | Select-Object -First 1
        }
        
        $pText = ""; $pCount = 1; $baseDmg = 0

        if ($ammoRow -and $ammoRow.DanoBase -match '^(\d+)(x\d+)?') {
            $baseDmg = [int]$Matches[1]
            if ($Matches[2]) {
                $pText = $Matches[2]
                $pCount = [int]($pText.Substring(1))
            }
        }
        ...
        $newCombo = [PSCustomObject]@{ Name=$comboName; Weapon=$wReal; Ammo=$aReal; Data=$parsed; Color=$null; Marker=$null; PelletText=$pText; PelletCount=$pCount; BaseDmg=$baseDmg }
        $script:ActiveChartCombos.Add($newCombo)
        
        script:Update-ChartEngine
    })

    $btnReset.Add_Click({
        $script:ActiveChartCombos.Clear()
        script:Update-ChartEngine
    })
...
function Start-WeaponEditor {
    ...
    if ($EditItem) { 
        $cmbCaliber.SelectedItem = $EditItem.Calibre 
        
        # O DETETIVE DA TRAVA INTELIGENTE (ARMA)
        $isWeaponInUse = $false
        $dmgCsvPath = Join-Path (Get-DatabasePath) "DamageRange.csv"
        if (Test-Path $dmgCsvPath) {
            $dmgData = @(Import-Csv $dmgCsvPath -Delimiter ";" -Encoding UTF8)
            foreach ($row in $dmgData) {
                if ($row.Arma -eq $EditItem.NomeItem) {
                    $isWeaponInUse = $true
                    break
                }
            }
        ...
        if ($isWeaponInUse) {
            $cmbCaliber.Enabled = $false # Tranca apenas se tiver histórico!
        }
    }
    ...
    $cmbModeQty.Add_DropDown({
        if ($this.Items.Count -gt 0 -and $this.Items[0] -eq "Selecione os dados aqui") { $this.Items.RemoveAt(0) }
    })
    ...
    $btnSave.Add_Click({
        ...
        if ($EditItem) { 
            for ($i = 0; $i -lt $allWeapons.Count; $i++) { 
                if ($allWeapons[$i].NomeItem -eq $EditItem.NomeItem) { 
                    $allWeapons[$i] = [PSCustomObject]$newItemData; break 
                } 
            } 
        } else { 
            $allWeapons += [PSCustomObject]$newItemData 
        }
        $allWeapons | Export-Csv -Path $csvPath -NoTypeInformation -Delimiter ";" -Encoding UTF8
        
        # =========================================================================
        # [CASCATA DE SEGURANÇA]: Atualiza o nome da Arma no DamageRange.csv
        # =========================================================================
        if ($EditItem -and $EditItem.NomeItem -ne $newName) {
            $dmgCsvPath = Join-Path (Get-DatabasePath) "DamageRange.csv"
            if (Test-Path $dmgCsvPath) {
                $dmgData = @(Import-Csv $dmgCsvPath -Delimiter ";" -Encoding UTF8)
                $dmgChanged = $false
                ...
                if ($dmgChanged) {
                    $dmgData | Export-Csv -Path $dmgCsvPath -NoTypeInformation -Delimiter ";" -Encoding UTF8
                }
            }
        }
        # =========================================================================
...
function Start-AmmoEditor {
    ...
    # 2. CALIBRE E TRAVA INTELIGENTE DUPLA
    $cmbCaliber = New-Object System.Windows.Forms.ComboBox; $cmbCaliber.DropDownStyle = "DropDownList"
    $calPath = Join-Path (Get-DatabasePath) "Caliber.csv"
    $calList = @()
    ...
    if ($EditItem) { 
        ...
        # O DETETIVE DA TRAVA INTELIGENTE DUPLA (MUNIÇÃO)
        $isAmmoInUse = $false
        $dmgCsvPath = Join-Path (Get-DatabasePath) "DamageRange.csv"
        if (Test-Path $dmgCsvPath) {
            $dmgData = @(Import-Csv $dmgCsvPath -Delimiter ";" -Encoding UTF8)
            foreach ($row in $dmgData) {
                if ($row."nome da munição" -eq $EditItem.NomeItem -and $row.Calibre -eq $EditItem.Calibre) {
                    $isAmmoInUse = $true
                    break
                }
            }
        ...
        if ($isAmmoInUse) {
            $cmbCaliber.Enabled = $false # Tranca apenas se tiver histórico com este nome E calibre exatos!
        }
    }
    
    Add-Field -LabelText "Qual é o calibre dessa munição?" -Control $cmbCaliber -Column 1

    # 3. Nível de Penetração
    ...
    $btnSave.Add_Click({
        ...
        # =========================================================================
        # [CASCATA DE SEGURANÇA]: Atualiza o nome da Munição no DamageRange.csv
        # =========================================================================
        if ($EditItem -and $EditItem.NomeItem -ne $newName) {
            $dmgCsvPath = Join-Path (Get-DatabasePath) "DamageRange.csv"
            if (Test-Path $dmgCsvPath) {
...
# ===================================================================================
# MÓDULO: GERENCIAR ALCANCE DE DANO (DAMAGE RANGE)
# ===================================================================================

function Get-DamageRangeFile {
    $base = ""
    if (Get-Command Get-DatabasePath -ErrorAction SilentlyContinue) {
        $base = Get-DatabasePath
    } else {
        if ($PSScriptRoot) { $base = Join-Path $PSScriptRoot "Database ABI" }
        else { $base = Join-Path (Get-Location).Path "Database ABI" }
    }
    
    $file = Join-Path $base "DamageRange.csv"
    return $file
}

function Save-DamageData {
    param ($WeaponName, $Calibre, $AmmoName, $Dano, $Alcance, $OldDano = $null, $OldAlcance = $null, $IsEdit = $false, $IsDelete = $false)
    try {
        $csvPath = Get-DamageRangeFile
        $allData = @()
        
        if (Test-Path $csvPath) { 
            $allData = @(Import-Csv $csvPath -Delimiter ";" -Encoding UTF8) 
        }
        
        # Encontra a linha específica desta arma e munição
        $targetRow = $allData | Where-Object { $_.Arma -eq $WeaponName -and $_."nome da munição" -eq $AmmoName } | Select-Object -First 1
        
        $pairs = @()
        if ($targetRow -and $targetRow."dano(alcance)") {
            $splits = $targetRow."dano(alcance)" -split '\|'
            foreach ($s in $splits) {
                $s = $s.Trim()
                if ($s -match '^(\d+)\((\d+)\)$') {
                    $pairs += [PSCustomObject]@{ Dano = [int]$matches[1]; Alcance = [int]$matches[2] }
                }
            }
        }
        
        # Aplica a Adição, Edição ou Remoção na lista de pares
        if ($IsDelete) {
            $pairs = @($pairs | Where-Object { -not ($_.Dano -eq $OldDano -and $_.Alcance -eq $OldAlcance) })
        } elseif ($IsEdit) {
            foreach ($p in $pairs) {
                if ($p.Dano -eq $OldDano -and $p.Alcance -eq $OldAlcance) {
                    $p.Dano = $Dano
                    $p.Alcance = $Alcance
                    break
                }
            ...
            $pairs = @($pairs)
            $pairs += [PSCustomObject]@{ Dano = $Dano; Alcance = $Alcance }
        }
        
        # Reordena: ALCANCE Crescente (Prioridade 1) e DANO Decrescente (Desempate)
        $pairs = @($pairs | Sort-Object -Property @{Expression={$_.Alcance}; Descending=$false}, @{Expression={$_.Dano}; Descending=$true})
        
        $newStr = ($pairs | ForEach-Object { "$($_.Dano)($($_.Alcance))" }) -join " | "
        
        # Atualiza a grelha de dados geral
        if ($targetRow) {
            if ($newStr) {
                $targetRow."dano(alcance)" = $newStr
            } else {
                # Se não sobrar dados, remove a linha inteira da arma/munição
                $allData = @($allData | Where-Object { -not ($_.Arma -eq $WeaponName -and $_."nome da munição" -eq $AmmoName) })
            }
        } else {
            if ($newStr) {
        ...
        if ($allData.Count -gt 0) {
            $allData | Sort-Object Arma, "nome da munição" | Export-Csv -Path $csvPath -NoTypeInformation -Delimiter ";" -Encoding UTF8
        } else {
            # Se o arquivo ficou vazio, recria o cabeçalho base limpo
            "Arma;Calibre;nome da munição;dano(alcance)" | Out-File -FilePath $csvPath -Encoding UTF8
        }
    } catch {
        [System.Windows.Forms.MessageBox]::Show("Erro ao salvar o arquivo CSV: $($_.Exception.Message)", "Erro Crítico", "OK", "Error")
    }
}

function Refresh-DamageGrid {
    param ($WeaponName, $AmmoName, $Grid)
    try {
        # Limpa o lixo da memória antes de criar uma tabela nova para evitar vazamento
        if ($Grid.DataSource -is [System.Data.DataTable]) {
            $Grid.DataSource.Dispose()
        }

        $csvPath = Get-DamageRangeFile
        
        # Recria as 3 colunas independentes para a Interface Visual
        $dt = New-Object System.Data.DataTable
        $dt.Columns.Add("Munição", [string])
        $dt.Columns.Add("Dano", [int])
        $dt.Columns.Add("Alcance (m)", [int])
        
        if (Test-Path $csvPath) {
            $data = @(Import-Csv $csvPath -Delimiter ";" -Encoding UTF8)
            # Busca a linha da arma e munição que o usuário selecionou na tela
            $row = $data | Where-Object { $_.Arma -eq $WeaponName -and $_."nome da munição" -eq $AmmoName } | Select-Object -First 1
            
            if ($row -and $row."dano(alcance)") {
                $splits = $row."dano(alcance)" -split '\|'
                foreach ($s in $splits) {
                    $s = $s.Trim()
                    if ($s -match '^(\d+)\((\d+)\)$') {
                        $dt.Rows.Add($row."nome da munição", [int]$matches[1], [int]$matches[2]) | Out-Null
                    }
                }
            }
        }
        ...
        # Bloqueia a ordenação ao clicar no cabeçalho
        foreach ($col in $Grid.Columns) {
            $col.SortMode = [System.Windows.Forms.DataGridViewColumnSortMode]::NotSortable
        }
    } catch {
        [System.Windows.Forms.MessageBox]::Show("Erro ao carregar a tabela: $($_.Exception.Message)", "Erro", "OK", "Error")
    }
}

function Show-DamageInputDialog {
    param ($Scenario = 1, $RefDamage = 0, $RefRange = 0, $ExistingPairs, $IsEditMode = $false, $CurrentDamage = "", $CurrentRange = "", $ShowVoltar = $false, $LockRange = $false)
    
    $dlg = New-Object System.Windows.Forms.Form
    $dlg.Size = New-Object System.Drawing.Size(700, 320)
    $dlg.StartPosition = "CenterParent"
    $dlg.BackColor = $global:theme.PanelBack
    $dlg.ForeColor = $global:theme.TextMain
    $dlg.FormBorderStyle = "FixedDialog"
    $dlg.MaximizeBox = $false
    $dlg.MinimizeBox = $false
    
    if ($Scenario -eq 1) {
        $lblDmgText = "Qual é o dano máximo inicial da munição (usando o melhor cano, caso exista)?"
        $lblRangeText = "Qual é o alcance limite para esse dano inicial onde o valor é constante e invariável?"
        $dlgTitle = "Dados Iniciais"
    } elseif ($Scenario -eq 2) {
        $lblDmgText = "Qual é o dano mínimo final da munição a partir de onde o valor se torna invariável?"
        $lblRangeText = "Qual é o alcance final (geralmente é o alcance inicial + 3 metros) onde o dano mínimo fica invariável?"
        $dlgTitle = "Dados Finais"
    } else {
        $lblDmgText = "Qual é o novo valor de queda de dano da munição?"
        $lblRangeText = "Qual é o novo alcance limite para esta queda de dano?"
        $dlgTitle = "Dados Intermediários"
    }
    
    if ($IsEditMode) {
        $lblDmgText = $lblDmgText -replace "Qual é o novo ", "Qual é o "
        $lblRangeText = $lblRangeText -replace "Qual é o novo ", "Qual é o "
        
        $lblDmgText = $lblDmgText -replace "Qual é o ", "Qual é o NOVO "
        $lblRangeText = $lblRangeText -replace "Qual é o ", "Qual é o NOVO "
        
        $dlgTitle = "Editar " + $dlgTitle
    } else {
        $dlgTitle = "Adicionar " + $dlgTitle
    }
    
    if ($LockRange) {
        $lblRangeText += " (Herdado da Arma)"
    }
    
    $dlg.Text = $dlgTitle
    
    $lblDmg = New-Object System.Windows.Forms.Label
    $lblDmg.Text = $lblDmgText
    $lblDmg.Location = "20, 20"; $lblDmg.AutoSize = $true; $lblDmg.ForeColor = $global:theme.OrangeAccent
    $lblDmg.MaximumSize = New-Object System.Drawing.Size(640, 0)
    
    $txtDmg = New-Object System.Windows.Forms.TextBox
    $txtDmg.Location = "20, 65"; $txtDmg.Width = 150; $txtDmg.BackColor = $global:theme.ButtonBack; $txtDmg.ForeColor = $global:theme.TextMain
    $txtDmg.Text = $CurrentDamage 
    
    $lblRange = New-Object System.Windows.Forms.Label
    $lblRange.Text = $lblRangeText
    $lblRange.Location = "20, 110"; $lblRange.AutoSize = $true; $lblRange.ForeColor = $global:theme.OrangeAccent
    $lblRange.MaximumSize = New-Object System.Drawing.Size(640, 0)
    
    $txtRange = New-Object System.Windows.Forms.TextBox
    $txtRange.Location = "20, 155"; $txtRange.Width = 150
    if ($LockRange) {
        $txtRange.BackColor = $global:theme.PanelBack
        $txtRange.ForeColor = $global:theme.TextDim
        $txtRange.Enabled = $false
    } else {
        $txtRange.BackColor = $global:theme.ButtonBack
        $txtRange.ForeColor = $global:theme.TextMain
        $txtRange.Enabled = $true
    }
    $txtRange.Text = $CurrentRange 
    
    $btnOk = New-Object System.Windows.Forms.Button
    $btnOk.Text = "Confirmar"; $btnOk.Location = "430, 225"; $btnOk.Size = "100, 35"; $btnOk.FlatStyle = "Flat"
    $btnOk.BackColor = $global:theme.GreenSuccess; $btnOk.ForeColor = $global:theme.Background; $btnOk.DialogResult = "OK"
    
    $btnCancel = New-Object System.Windows.Forms.Button
    $btnCancel.Location = "550, 225"; $btnCancel.Size = "100, 35"; $btnCancel.FlatStyle = "Flat"
    
    if ($ShowVoltar) {
        $btnCancel.Text = "Voltar"
        $btnCancel.DialogResult = "Retry"
        $btnCancel.BackColor = $global:theme.OrangeAccent
        $btnCancel.ForeColor = $global:theme.Background
    } else {
        $btnCancel.Text = "Cancelar"
        $btnCancel.DialogResult = "Cancel"
        $btnCancel.BackColor = $global:theme.ButtonBack
        $btnCancel.ForeColor = $global:theme.TextMain
    }
    
    $dlg.Controls.AddRange(@($lblDmg, $txtDmg, $lblRange, $txtRange, $btnOk, $btnCancel))
    $dlg.AcceptButton = $btnOk
    $dlg.CancelButton = $btnCancel
    
    $script:DamageInputResult = $null
    
    $btnOk.Add_Click({
        $d = 0; $r = 0
        if (-not [int]::TryParse($txtDmg.Text, [ref]$d) -or -not [int]::TryParse($txtRange.Text, [ref]$r)) {
            [System.Windows.Forms.MessageBox]::Show("Insira apenas valores numéricos válidos e inteiros.", "Erro de Formatação", "OK", "Warning")
            $dlg.DialogResult = "None"
            return
        }
        
        if ($null -ne $ExistingPairs) {
            foreach ($pair in $ExistingPairs) {
                if ($pair.Dano -eq $d -and $pair.Alcance -eq $r) {
                    [System.Windows.Forms.MessageBox]::Show("Este dado já existe! A combinação de Dano $d e Alcance $r já está registrada para esta munição.", "Aviso", "OK", "Warning")
                    $dlg.DialogResult = "None"
                    return
                }
            }
        ...
        if ($Scenario -gt 1 -and -not $IsEditMode) {
            $erroDano = ($d -gt $RefDamage)
            $erroAlcance = ($r -lt $RefRange)
            
            if ($erroDano -and $erroAlcance) {
                $mensagemDupla = "Foram detectadas duas violações em relação à Referência Anterior:`n`n"
                $mensagemDupla += "1. O novo dano ($d) não pode ser MAIOR que o Dano de Referência ($RefDamage).`n"
                $mensagemDupla += "2. O novo alcance ($r) não pode ser MENOR que o Alcance de Referência ($RefRange)."
                [System.Windows.Forms.MessageBox]::Show($mensagemDupla, "Aviso Balístico Múltiplo", "OK", "Warning")
                $dlg.DialogResult = "None"
                return
            } elseif ($erroDano) {
                [System.Windows.Forms.MessageBox]::Show("O novo dano ($d) não pode ser MAIOR que o Dano de Referência ($RefDamage).", "Aviso Balístico", "OK", "Warning")
                $dlg.DialogResult = "None"
                return
            } elseif ($erroAlcance) {
                [System.Windows.Forms.MessageBox]::Show("O novo alcance ($r) não pode ser MENOR que o Alcance de Referência ($RefRange).", "Aviso Balístico", "OK", "Warning")
                $dlg.DialogResult = "None"
                return
            }
        }
        $script:DamageInputResult = @{ Dano = $d; Alcance = $r }
    })
    
    $dr = $dlg.ShowDialog()
    $script:LastDialogAction = $dr.ToString() 
    
    if ($script:LastDialogAction -eq "OK") {
        $res = $script:DamageInputResult
        $dlg.Dispose()
        return $res
    }
...
function Build-DamageRangePanels {
    param ($ui)
    
    if ($ui -is [array]) { $ui = $ui | Where-Object { $_ -is [hashtable] } | Select-Object -First 1 }
    
    if ($null -ne $ui["PnlDamageMain"]) { 
        $ui["PnlDamageMain"].Visible = $true
        $ui.PnlManagerRoot.Controls.Find("PnlTutorial", $true)[0].Visible = $true
        $ui.PnlManagerRoot.Controls.Find("PnlMain", $true)[0].Visible = $false
        return 
    }
    ...
    # CONTAINER PRINCIPAL
    # ==========================================
    $pnlContainer = New-Object System.Windows.Forms.Panel
    $pnlContainer.Dock = "Fill"
    $pnlContainer.BackColor = $global:theme.Background
    $pnlContainer.Visible = $false
    
    # ==========================================
    # TELA DE TUTORIAL (A Barreira)
    # ==========================================
    $pnlTutorial = New-Object System.Windows.Forms.Panel
    $pnlTutorial.Name = "PnlTutorial"
    $pnlTutorial.Dock = "Fill"
    $pnlTutorial.BackColor = $global:theme.Background
    $pnlTutorial.Visible = $true 
    
    $lblTutTitle = New-Object System.Windows.Forms.Label
    $lblTutTitle.Text = "Gerenciar Alcance de Dano - Instruções Essenciais"
    $lblTutTitle.Font = $script:fontMngSubTitle
    $lblTutTitle.ForeColor = $global:theme.OrangeAccent
    $lblTutTitle.AutoSize = $true
    $lblTutTitle.Location = "50, 30"
    
    $rtbTut = New-Object System.Windows.Forms.RichTextBox
    $rtbTut.Location = "50, 80"
    $rtbTut.Size = "960, 560"
    $rtbTut.BackColor = $global:theme.PanelBack
    $rtbTut.ForeColor = $global:theme.TextMain
    $rtbTut.ReadOnly = $true
    $rtbTut.BorderStyle = "None"
    
    $btnTutVoltar = New-Object System.Windows.Forms.Button
    $btnTutVoltar.Text = "Voltar ao Menu"
    $btnTutVoltar.Location = "50, 670"
    $btnTutVoltar.Size = "200, 45"
    $btnTutVoltar.FlatStyle = "Flat"
    $btnTutVoltar.BackColor = $global:theme.ButtonBack
    $btnTutVoltar.ForeColor = $global:theme.TextMain
    
    $btnTutProsseguir = New-Object System.Windows.Forms.Button
    $btnTutProsseguir.Text = "Prosseguir"
    $btnTutProsseguir.Location = "810, 670"
    $btnTutProsseguir.Size = "200, 45"
    $btnTutProsseguir.FlatStyle = "Flat"
    $btnTutProsseguir.BackColor = $global:theme.GreenSuccess
    $btnTutProsseguir.ForeColor = $global:theme.Background
    
    $pnlTutorial.Controls.AddRange(@($lblTutTitle, $rtbTut, $btnTutVoltar, $btnTutProsseguir))
    
    # Configuração de Cores e Texto do Tutorial
    $cInit = $global:theme.GreenSuccess
    $cInter = $global:theme.OrangeAccent
    $cFinal = $global:theme.RedAlert
    $cMain = $global:theme.TextMain
    $cDim = $global:theme.TextDim

    $fReg = New-Object System.Drawing.Font("Segoe UI", 11)
    $fBold = New-Object System.Drawing.Font("Segoe UI", 11, [System.Drawing.FontStyle]::Bold)
    $fMono = New-Object System.Drawing.Font("Consolas", 12)

    $AppendRtb = {
        param($Text, $Color, $Font)
        $rtbTut.SelectionStart = $rtbTut.TextLength
        $rtbTut.SelectionLength = 0
        $rtbTut.SelectionColor = $Color
        if ($Font) { $rtbTut.SelectionFont = $Font }
        $rtbTut.AppendText($Text)
    }

    & $AppendRtb "Bem-vindo ao Gerenciador de Balística`n`n" $cInter $fBold
    
    & $AppendRtb "Antes de inserir os dados, é fundamental entender como a física do jogo é calculada em nosso banco de dados.`n`n" $cMain $fReg
    
    & $AppendRtb "A Regra de Ouro:`n" $cMain $fBold
    & $AppendRtb "A " $cMain $fReg
    & $AppendRtb "ARMA" $cInter $fBold
    & $AppendRtb " é quem dita o Alcance (a distância que o tiro percorre antes de perder força). A " $cMain $fReg
    & $AppendRtb "MUNIÇÃO" $cInter $fBold
    & $AppendRtb " dita apenas o Dano. Portanto, ao alterar a distância de uma munição, todas as outras munições daquela mesma arma serão sincronizadas automaticamente.`n`n" $cMain $fReg
    
    & $AppendRtb "Os 3 Tipos de Dados:`n" $cMain $fBold
    & $AppendRtb "• Dado Inicial: É o teto da balística. Mostra o dano máximo da bala e a distância limite onde ela começa a perder força.`n" $cMain $fReg
    & $AppendRtb "• Dados Intermediários: São os 'degraus' de queda de dano que acontecem no espaço entre o Inicial e o Final.`n" $cMain $fReg
    & $AppendRtb "• Dado Final: É o chão da balística. Mostra a distância final onde a bala atinge seu dano mínimo e se estabiliza.`n`n" $cMain $fReg

    # --- A NOVA EXPLICAÇÃO DO MODELO 1 ---
    & $AppendRtb "A Regra da Estabilidade Absoluta:`n" $cMain $fBold
    & $AppendRtb "O jogo esconde os decimais da distância (ex: 30.1m, 30.5m). Para um Dano Inicial ou Final ser considerado verdadeiramente 'estável', o valor de dano deve ser exatamente o mesmo durante todo aquele metro (do 30.0m ao 30.9m). Se houver qualquer flutuação dentro desse mesmo metro inteiro, ele ainda é um Dado Intermediário.`n`n" $cMain $fReg

    & $AppendRtb "Exemplo Visual:`n" $cMain $fBold
    & $AppendRtb "`n" $cMain $fMono
    
    # Gráfico - Linha 1 (DANO)
    & $AppendRtb "DANO:   " $cMain $fMono
    & $AppendRtb " 74--- 74--- 74---" $cInit $fMono
    & $AppendRtb " 72--- 70--- 66--- 64--- 60--- 59--- 57--- 52---" $cInter $fMono
    & $AppendRtb " 52--- 52--- 52   `n" $cFinal $fMono

    # Gráfico - Linha 2 (PIPES)
    & $AppendRtb "        " $cMain $fMono
    & $AppendRtb " |     |     |    " $cDim $fMono
    & $AppendRtb " |     |     |     |     |     |     |     |    " $cDim $fMono
    & $AppendRtb " |     |     |    `n" $cDim $fMono

    # Gráfico - Linha 3 (METROS)
    & $AppendRtb "METROS: " $cMain $fMono
    & $AppendRtb " 0m    11m   12m  " $cInit $fMono
    & $AppendRtb " 12m   12m   12m   12m   12m   12m   13m   13m  " $cInter $fMono
    & $AppendRtb " 14m   25m   ∞    `n`n" $cFinal $fMono

    # Legenda Colorida
    & $AppendRtb "Legenda:`n" $cMain $fBold
    & $AppendRtb "■ INICIAL: " $cInit $fBold
    & $AppendRtb "O teto da balística. Força máxima e constante antes da queda.`n" $cMain $fReg
    & $AppendRtb "■ INTERMEDIÁRIOS: " $cInter $fBold
    & $AppendRtb "A zona de transição e do arredondamento (caos dos décimos).`n" $cMain $fReg
    & $AppendRtb "■ FINAL: " $cFinal $fBold
    & $AppendRtb "O chão da balística. Dano mínimo, estabilizado e permanente.`n" $cMain $fReg

    $rtbTut.Select(0,0)

    # ==========================================
    # TELA PRINCIPAL DE EDIÇÃO (A Tabela)
    # ==========================================
    $pnlMainInterno = New-Object System.Windows.Forms.Panel
    $pnlMainInterno.Name = "PnlMain"
    $pnlMainInterno.Dock = "Fill"
    $pnlMainInterno.BackColor = $global:theme.Background
    $pnlMainInterno.Visible = $false
    
    $lblTitle = New-Object System.Windows.Forms.Label; $lblTitle.Text = "Gerenciar Alcance de Dano"; $lblTitle.Font = $script:fontMngSubTitle; $lblTitle.ForeColor = $global:theme.OrangeAccent; $lblTitle.AutoSize = $true; $lblTitle.Location = "50, 30"
    $btnBack = New-Object System.Windows.Forms.Button; $btnBack.Text = "Voltar"; $btnBack.Location = "950, 30"; $btnBack.Size = "100, 30"; $btnBack.FlatStyle = "Flat"; $btnBack.BackColor = $global:theme.ButtonBack; $btnBack.ForeColor = $global:theme.TextDim
    
    $lblClass = New-Object System.Windows.Forms.Label; $lblClass.Text = "Escolha a Classe de Arma:"; $lblClass.Location = "50, 100"; $lblClass.AutoSize = $true; $lblClass.ForeColor = $global:theme.TextMain
    $cbClass = New-Object System.Windows.Forms.ComboBox; $cbClass.Location = "50, 125"; $cbClass.Width = 300; $cbClass.DropDownStyle = "DropDownList"; $cbClass.BackColor = $global:theme.ButtonBack; $cbClass.ForeColor = $global:theme.TextMain
    
    $lblWeapon = New-Object System.Windows.Forms.Label; $lblWeapon.Text = "Escolha a Arma:"; $lblWeapon.Location = "380, 100"; $lblWeapon.AutoSize = $true; $lblWeapon.ForeColor = $global:theme.TextMain
    $cbWeapon = New-Object System.Windows.Forms.ComboBox; $cbWeapon.Location = "380, 125"; $cbWeapon.Width = 300; $cbWeapon.DropDownStyle = "DropDownList"; $cbWeapon.BackColor = $global:theme.ButtonBack; $cbWeapon.ForeColor = $global:theme.TextMain; $cbWeapon.Enabled = $false
    
    $lblAmmo = New-Object System.Windows.Forms.Label; $lblAmmo.Text = "Escolha a Munição:"; $lblAmmo.Location = "710, 100"; $lblAmmo.AutoSize = $true; $lblAmmo.ForeColor = $global:theme.TextMain
    $cbAmmo = New-Object System.Windows.Forms.ComboBox; $cbAmmo.Location = "710, 125"; $cbAmmo.Width = 300; $cbAmmo.DropDownStyle = "DropDownList"; $cbAmmo.BackColor = $global:theme.ButtonBack; $cbAmmo.ForeColor = $global:theme.TextMain; $cbAmmo.Enabled = $false
    
    $btnAddData = New-Object System.Windows.Forms.Button; $btnAddData.Text = "Adicionar os Dados"; $btnAddData.Location = "50, 185"; $btnAddData.Size = "300, 45"; $btnAddData.FlatStyle = "Flat"; $btnAddData.BackColor = $global:theme.OrangeAccent; $btnAddData.ForeColor = $global:theme.Background; $btnAddData.Enabled = $false
    
    $dgvDmg = New-Object System.Windows.Forms.DataGridView; $dgvDmg.Location = "50, 250"; $dgvDmg.Size = "960, 400"; $dgvDmg.AllowUserToAddRows = $false; $dgvDmg.ReadOnly = $true; $dgvDmg.SelectionMode = "FullRowSelect"; $dgvDmg.MultiSelect = $false; $dgvDmg.RowHeadersVisible = $false; $dgvDmg.AutoSizeColumnsMode = "Fill"
    $dgvDmg.BackgroundColor = $global:theme.PanelBack; $dgvDmg.DefaultCellStyle.BackColor = $global:theme.Background; $dgvDmg.DefaultCellStyle.ForeColor = $global:theme.TextMain
    $dgvDmg.AllowUserToResizeColumns = $false
    $dgvDmg.AllowUserToResizeRows = $false
    $dgvDmg.ColumnHeadersHeightSizeMode = [System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode]::DisableResizing
	$dgvDmg.EnableHeadersVisualStyles = $false
    $dgvDmg.ColumnHeadersDefaultCellStyle.SelectionBackColor = $dgvDmg.ColumnHeadersDefaultCellStyle.BackColor
    $dgvDmg.ColumnHeadersDefaultCellStyle.SelectionForeColor = $dgvDmg.ColumnHeadersDefaultCellStyle.ForeColor
    
    $btnRemDmg = New-Object System.Windows.Forms.Button; $btnRemDmg.Text = "Remover Dado"; $btnRemDmg.Location = "50, 670"; $btnRemDmg.Size = "200, 45"; $btnRemDmg.FlatStyle = "Flat"; $btnRemDmg.BackColor = $global:theme.RedAlert; $btnRemDmg.ForeColor = "White"; $btnRemDmg.Enabled = $false
    $btnEditDmg = New-Object System.Windows.Forms.Button; $btnEditDmg.Text = "Editar Dado"; $btnEditDmg.Location = "270, 670"; $btnEditDmg.Size = "200, 45"; $btnEditDmg.FlatStyle = "Flat"; $btnEditDmg.BackColor = $global:theme.ButtonBack; $btnEditDmg.ForeColor = $global:theme.TextMain; $btnEditDmg.Enabled = $false
    
    $pnlMainInterno.Controls.AddRange(@($lblTitle, $btnBack, $lblClass, $cbClass, $lblWeapon, $cbWeapon, $lblAmmo, $cbAmmo, $btnAddData, $dgvDmg, $btnRemDmg, $btnEditDmg))
    
    $pnlContainer.Controls.Add($pnlTutorial)
    $pnlContainer.Controls.Add($pnlMainInterno)
    $ui.PnlManagerRoot.Controls.Add($pnlContainer)
    $ui["PnlDamageMain"] = $pnlContainer

    # ==========================================
    # LÓGICA DE NAVEGAÇÃO DOS BOTÕES
    # ==========================================
    $btnTutVoltar.Add_Click({
        $ui["PnlDamageMain"].Visible = $false
        $ui.PnlMenu.Visible = $true
    }.GetNewClosure())

    $btnTutProsseguir.Add_Click({
        $pnlTutorial.Visible = $false
        $pnlMainInterno.Visible = $true
    }.GetNewClosure())
    ...
    # ==========================================
    $cbClass.Items.Clear(); $cbClass.Items.Add("-- Selecione --") | Out-Null
    $global:WeaponClassToPortugueseMap.Values | Select-Object -Unique | Sort-Object | ForEach-Object { $cbClass.Items.Add($_) | Out-Null }
    $cbClass.SelectedIndex = 0

    $dgvDmg.Add_SelectionChanged({ 
        $btnRemDmg.Enabled = ($dgvDmg.SelectedRows.Count -gt 0)
        $btnEditDmg.Enabled = ($dgvDmg.SelectedRows.Count -gt 0) 
    }.GetNewClosure())

    $cbClass.Add_DropDown({ if ($cbClass.Items.Contains("-- Selecione --")) { $cbClass.Items.Remove("-- Selecione --") } }.GetNewClosure())
    $cbWeapon.Add_DropDown({ if ($cbWeapon.Items.Contains("-- Selecione --")) { $cbWeapon.Items.Remove("-- Selecione --") } }.GetNewClosure())
    $cbAmmo.Add_DropDown({ if ($cbAmmo.Items.Contains("-- Selecione --")) { $cbAmmo.Items.Remove("-- Selecione --") } }.GetNewClosure())

    $cbClass.Add_SelectedIndexChanged({
        $cbWeapon.Items.Clear(); $cbWeapon.Enabled = $false
        $cbAmmo.Items.Clear(); $cbAmmo.Enabled = $false
        $btnAddData.Enabled = $false
        if ($dgvDmg.DataSource -is [System.Data.DataTable]) { $dgvDmg.DataSource.Dispose() }
        $dgvDmg.DataSource = $null
        $selClass = $cbClass.SelectedItem
        if ($null -ne $selClass -and $selClass -ne "-- Selecione --") {
            $cbWeapon.Items.Add("-- Selecione --") | Out-Null
            $weps = Get-WeaponData | Where-Object { $_.ClasseDisplay -eq $selClass } | Sort-Object Nome
            foreach ($w in $weps) { $cbWeapon.Items.Add($w.Nome) | Out-Null }
            $cbWeapon.Enabled = $true; $cbWeapon.SelectedIndex = 0
        }
    }.GetNewClosure())

    $cbWeapon.Add_SelectedIndexChanged({
        $cbAmmo.Items.Clear(); $cbAmmo.Enabled = $false; $btnAddData.Enabled = $false
        if ($dgvDmg.DataSource -is [System.Data.DataTable]) { $dgvDmg.DataSource.Dispose() }
        $dgvDmg.DataSource = $null
        $selWep = $cbWeapon.SelectedItem
        if ($null -ne $selWep -and $selWep -ne "-- Selecione --") {
            $cbAmmo.Items.Add("-- Selecione --") | Out-Null
            $selWepData = Get-WeaponData | Where-Object { $_.Nome -eq $selWep } | Select-Object -First 1
            if ($selWepData) {
                $ammos = Get-AmmoData | Where-Object { $_.Calibre -eq $selWepData.Calibre } | Sort-Object Nome
                foreach ($a in $ammos) { $cbAmmo.Items.Add($a.Nome) | Out-Null }
                $cbAmmo.Enabled = $true; $cbAmmo.SelectedIndex = 0
            }
        }
    }.GetNewClosure())

    $cbAmmo.Add_SelectedIndexChanged({
        $selAmmo = $cbAmmo.SelectedItem
        if ($null -ne $selAmmo -and $selAmmo -ne "-- Selecione --") {
            Refresh-DamageGrid -WeaponName $cbWeapon.SelectedItem -AmmoName $selAmmo -Grid $dgvDmg
            
            # [NOVO: Bloqueia o botão se já existirem 2 dados]
            if ($dgvDmg.DataSource -and $dgvDmg.DataSource.Rows.Count -ge 2) { 
                $btnAddData.Enabled = $false 
            } else { 
    ...
    # ==========================================
    # LÓGICA INLINE DOS BOTÕES
    # ==========================================

    $btnAddData.Add_Click({
        try {
            $selWep = Get-WeaponData | Where-Object { $_.Nome -eq $cbWeapon.SelectedItem } | Select-Object -First 1
            $calibre = if ($selWep) { $selWep.Calibre } else { "Desconhecido" }
            
            $dt = $dgvDmg.DataSource
            $isInitial = ($null -eq $dt -or $dt.Rows.Count -eq 0)
            
            # [SEGURANÇA EXTRA: Failsafe para impedir mais de 2 linhas]
            if (-not $isInitial -and $dt.Rows.Count -ge 2) {
                [System.Windows.Forms.MessageBox]::Show("O limite de dados (Inicial e Final) já foi atingido.", "Aviso", "OK", "Information")
                return
            }

            $existingPairs = @()
            if (-not $isInitial) {
                foreach ($r in $dt.Rows) { $existingPairs += [PSCustomObject]@{ Dano = [int]$r["Dano"]; Alcance = [int]$r["Alcance (m)"] } }
            }

            # Procura a herança de Alcance (Sincronização por Arma)
            $wepRanges = $null
            $csvPath = Get-DamageRangeFile
            $csv = Import-Csv -Path $csvPath -Delimiter ";" -Encoding UTF8 -ErrorAction SilentlyContinue
            if ($csv) {
                $otherAmmos = @($csv | Where-Object { $_.Arma -eq $cbWeapon.SelectedItem -and $_."nome da munição" -ne $cbAmmo.SelectedItem })
                if ($otherAmmos.Count -gt 0) {
                    $refRow = $otherAmmos[0] 
                    $splits = $refRow."dano(alcance)" -split '\|'
                    if ($splits.Count -ge 2) {
                        $firstSplit = $splits[0].Trim()
                        $lastSplit = $splits[-1].Trim()
                        
                        $initR = 0; $finalR = 0
                        if ($firstSplit -match '\((\d+)\)') { $initR = [int]$matches[1] }
                        if ($lastSplit -match '\((\d+)\)') { $finalR = [int]$matches[1] }
                        
                        if ($initR -gt 0 -and $finalR -gt 0) {
                            $wepRanges = @{ InitRange = $initR; FinalRange = $finalR }
                        }
                    }
                }
            }
            ...
            if ($isInitial) {
                if ($wepRanges) {
                    $res1 = Show-DamageInputDialog -Scenario 1 -RefDamage 0 -RefRange 0 -ExistingPairs $existingPairs -CurrentRange $wepRanges.InitRange -LockRange $true
                } else {
                    $res1 = Show-DamageInputDialog -Scenario 1 -RefDamage 0 -RefRange 0 -ExistingPairs $existingPairs
                }
                if ($res1) {
                    Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $cbAmmo.SelectedItem -Dano $res1.Dano -Alcance $res1.Alcance
                }
            } else {
                # [LIMPEZA: Como só há 2 dados, sabemos que aqui é sempre o Cenário 2 (Final)]
                $baseRow = $dt.Rows[0]
                $refDano = [int]$baseRow["Dano"]
                $refRange = [int]$baseRow["Alcance (m)"]
                
                $sugDano = [Math]::Round(($refDano * 0.70), [MidpointRounding]::AwayFromZero)
                if ($wepRanges) {
                    $res = Show-DamageInputDialog -Scenario 2 -RefDamage $refDano -RefRange $refRange -ExistingPairs $existingPairs -CurrentDamage $sugDano -CurrentRange $wepRanges.FinalRange -LockRange $true
                } else {
                    $sugRange = $refRange + 3
                    $res = Show-DamageInputDialog -Scenario 2 -RefDamage $refDano -RefRange $refRange -ExistingPairs $existingPairs -CurrentDamage $sugDano -CurrentRange $sugRange
                }
                
                if ($res) {
                    Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $cbAmmo.SelectedItem -Dano $res.Dano -Alcance $res.Alcance
                }
            }
            
            # Recarrega a grelha e atualiza o estado do botão
            Refresh-DamageGrid -WeaponName $cbWeapon.SelectedItem -AmmoName $cbAmmo.SelectedItem -Grid $dgvDmg
            if ($dgvDmg.DataSource -and $dgvDmg.DataSource.Rows.Count -ge 2) { $btnAddData.Enabled = $false } else { $btnAddData.Enabled = $true }
            
        } catch {
            [System.Windows.Forms.MessageBox]::Show("Erro no botão Adicionar Dados: $($_.Exception.Message)", "Erro", "OK", "Error")
        }
    }.GetNewClosure())

    $btnRemDmg.Add_Click({
        try {
            if ($dgvDmg.SelectedRows.Count -eq 0) { return }
            $row = $dgvDmg.SelectedRows[0]
            $selWep = Get-WeaponData | Where-Object { $_.Nome -eq $cbWeapon.SelectedItem } | Select-Object -First 1
            $calibre = if ($selWep) { $selWep.Calibre } else { "Desconhecido" }
            
            $oldDano = [int]$row.Cells["Dano"].Value
            $oldAlcance = [int]$row.Cells["Alcance (m)"].Value
            $dt = $dgvDmg.DataSource
            $totalRows = $dt.Rows.Count
            $selIndex = $row.Index
            
            # [LIMPEZA: Removidas as regras de intermediários, fica apenas a regra sagrada do Inicial/Final]
            if ($totalRows -eq 2 -and $selIndex -eq 0) {
                [System.Windows.Forms.MessageBox]::Show("O Dado Inicial é a fundação da balística. Tem de remover o Dado Final primeiro antes de apagar o Inicial.", "Remoção Bloqueada", "OK", "Warning")
                return
            }
            
            $msg = "Tem a certeza que deseja remover permanentemente este dado da munição $($cbAmmo.SelectedItem)?`n`nAlcance: $oldAlcance m`nDano: $oldDano"
            $ans = [System.Windows.Forms.MessageBox]::Show($msg, "Confirmar Remoção", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Warning)
            
            if ($ans -eq "Yes") {
                Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $cbAmmo.SelectedItem -OldDano $oldDano -OldAlcance $oldAlcance -IsDelete $true
                Refresh-DamageGrid -WeaponName $cbWeapon.SelectedItem -AmmoName $cbAmmo.SelectedItem -Grid $dgvDmg
                
                # Desbloqueia o botão porque agora há espaço na tabela
                if ($dgvDmg.DataSource -and $dgvDmg.DataSource.Rows.Count -ge 2) { $btnAddData.Enabled = $false } else { $btnAddData.Enabled = $true }
            }
        } catch {
            [System.Windows.Forms.MessageBox]::Show("Erro ao remover dado: $($_.Exception.Message)", "Erro", "OK", "Error")
        }
    }.GetNewClosure())

    $btnEditDmg.Add_Click({
        try {
            if ($dgvDmg.SelectedRows.Count -eq 0) { return }
            $row = $dgvDmg.SelectedRows[0]
            $selWep = Get-WeaponData | Where-Object { $_.Nome -eq $cbWeapon.SelectedItem } | Select-Object -First 1
            ...
            $dt = $dgvDmg.DataSource
            $isFirstInitial = ($row.Index -eq 0)
            
            $origInitDano = [int]$dt.Rows[0]["Dano"]; $origInitAlcance = [int]$dt.Rows[0]["Alcance (m)"]
            
            $hasFinal = ($dt.Rows.Count -gt 1)
            $origFinalDano = if ($hasFinal) { [int]$dt.Rows[1]["Dano"] } else { 0 }
            $origFinalAlcance = if ($hasFinal) { [int]$dt.Rows[1]["Alcance (m)"] } else { 0 }
            
            $tempInitDano = $origInitDano; $tempInitAlcance = $origInitAlcance
            $tempFinalDano = $origFinalDano; $tempFinalAlcance = $origFinalAlcance
            
            # [LIMPEZA: Removidas todas as procuras por "Extras/Intermediários"]
            $otherAmmosToUpdate = @()
            $csvPath = Get-DamageRangeFile
            $csv = Import-Csv -Path $csvPath -Delimiter ";" -Encoding UTF8 -ErrorAction SilentlyContinue
            if ($csv) {
                $otherAmmos = @($csv | Where-Object { $_.Arma -eq $cbWeapon.SelectedItem -and $_."nome da munição" -ne $cbAmmo.SelectedItem })
                if ($otherAmmos.Count -gt 0) {
                    foreach ($oa in $otherAmmos) {
                        $splits = $oa."dano(alcance)" -split '\|'
                        if ($splits.Count -ge 2) {
                            $firstSplit = $splits[0].Trim()
                            $lastSplit = $splits[-1].Trim()
                            ...
                            if ($firstSplit -match '^(\d+)\((\d+)\)') { $initD = [int]$matches[1]; $initR = [int]$matches[2] }
                            if ($lastSplit -match '^(\d+)\((\d+)\)') { $finalD = [int]$matches[1]; $finalR = [int]$matches[2] }
                            
                            if ($initR -gt 0) {
                                $otherAmmosToUpdate += [PSCustomObject]@{
                                    AmmoName = $oa."nome da munição"
                                    InitOldDano = $initD; InitOldAlcance = $initR
                                    FinalOldDano = $finalD; FinalOldAlcance = $finalR
                                }
                            }
                        }
                    }
            ...
            $state = 1
            while ($state -gt 0 -and $state -lt 4) {
                $epInit = @(); if ($hasFinal) { $epInit += [PSCustomObject]@{ Dano = $tempFinalDano; Alcance = $tempFinalAlcance } }
                $epFinal = @(); $epFinal += [PSCustomObject]@{ Dano = $tempInitDano; Alcance = $tempInitAlcance }
                
                if ($state -eq 1) { 
                    if ($isFirstInitial) {
                        $res = Show-DamageInputDialog -Scenario 1 -IsEditMode $true -CurrentDamage $tempInitDano -CurrentRange $tempInitAlcance -ShowVoltar $false -ExistingPairs $epInit -LockRange $false
                        if ($res) { $tempInitDano = $res.Dano; $tempInitAlcance = $res.Alcance; $state = 2 } else { $state = 0 }
                    } else {
                        $res = Show-DamageInputDialog -Scenario 2 -IsEditMode $true -CurrentDamage $tempFinalDano -CurrentRange $tempFinalAlcance -ShowVoltar $false -ExistingPairs $epFinal -LockRange $false
                        if ($res) { $tempFinalDano = $res.Dano; $tempFinalAlcance = $res.Alcance; $state = 2 } else { $state = 0 }
                    }
                }
                elseif ($state -eq 2) { 
                    if ($isFirstInitial) {
                        if ($hasFinal) {
                            $sugFinalDano = [Math]::Round(($tempInitDano * 0.70), [MidpointRounding]::AwayFromZero)
                            $sugFinalAlcance = $tempInitAlcance + 3
                            
                            $res = Show-DamageInputDialog -Scenario 2 -IsEditMode $true -CurrentDamage $sugFinalDano -CurrentRange $sugFinalAlcance -ShowVoltar $true -ExistingPairs $epFinal -LockRange $false
                            if ($res) {
                                $tempFinalDano = $res.Dano; $tempFinalAlcance = $res.Alcance; $state = 3
                            } elseif ($script:LastDialogAction -eq "Retry") { $state = 1 } else { $state = 0 }
                        } else {
                            $state = 3 # Se não há dado final, vai direto salvar
                        }
                    } else {
                        $sugInitDano = [Math]::Round(($tempFinalDano / 0.70), [MidpointRounding]::AwayFromZero)
                        $sugInitAlcance = $tempFinalAlcance - 3; if ($sugInitAlcance -lt 0) { $sugInitAlcance = 0 }
                        
                        $res = Show-DamageInputDialog -Scenario 1 -IsEditMode $true -CurrentDamage $sugInitDano -CurrentRange $sugInitAlcance -ShowVoltar $true -ExistingPairs $epInit -LockRange $false
                        if ($res) {
                            $tempInitDano = $res.Dano; $tempInitAlcance = $res.Alcance; $state = 3
                        } elseif ($script:LastDialogAction -eq "Retry") { $state = 1 } else { $state = 0 }
                    }
                }
                elseif ($state -eq 3) {
                    if ($hasFinal -and ($tempInitDano -lt $tempFinalDano -or $tempInitAlcance -gt $tempFinalAlcance)) {
                        [System.Windows.Forms.MessageBox]::Show("Paradoxo Crítico: O Dano Inicial não pode ser menor que o Final, e o Alcance Inicial não pode ser maior que o Final. Por favor, corrija os valores.", "Erro de Gráfico", "OK", "Error")
                        $state = 2
                        continue
                    }
                    $state = 4
            ...
            if ($state -eq 4) { 
                if ($tempInitDano -ne $origInitDano -or $tempInitAlcance -ne $origInitAlcance) {
                    Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $cbAmmo.SelectedItem -Dano $tempInitDano -Alcance $tempInitAlcance -OldDano $origInitDano -OldAlcance $origInitAlcance -IsEdit $true
                }
                if ($hasFinal -and ($tempFinalDano -ne $origFinalDano -or $tempFinalAlcance -ne $origFinalAlcance)) {
                    Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $cbAmmo.SelectedItem -Dano $tempFinalDano -Alcance $tempFinalAlcance -OldDano $origFinalDano -OldAlcance $origFinalAlcance -IsEdit $true
                }
                
                if ($tempInitAlcance -ne $origInitAlcance -or ($hasFinal -and $tempFinalAlcance -ne $origFinalAlcance)) {
                    foreach ($oa in $otherAmmosToUpdate) {
                        if ($tempInitAlcance -ne $oa.InitOldAlcance) {
                            Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $oa.AmmoName -Dano $oa.InitOldDano -Alcance $tempInitAlcance -OldDano $oa.InitOldDano -OldAlcance $oa.InitOldAlcance -IsEdit $true
                        }
                        if ($hasFinal -and $tempFinalAlcance -ne $oa.FinalOldAlcance) {
                            Save-DamageData -WeaponName $cbWeapon.SelectedItem -Calibre $calibre -AmmoName $oa.AmmoName -Dano $oa.FinalOldDano -Alcance $tempFinalAlcance -OldDano $oa.FinalOldDano -OldAlcance $oa.FinalOldAlcance -IsEdit $true
                        }
                    }
                }
            ...
            [System.Windows.Forms.MessageBox]::Show("Erro ao editar dado: $($_.Exception.Message)", "Erro", "OK", "Error")
        }
    }.GetNewClosure())
...
function Initialize-ManagerVisuals {
    ...
    # [ADICIONADO] Botão de Gerenciar Alcance de Dano
    $btnDamageRange = New-Object System.Windows.Forms.Button; $btnDamageRange.Text = "Gerenciar Alcance de Dano"; $btnDamageRange.Location = "390, 410"; $btnDamageRange.Size = $btnStyle.Size; $btnDamageRange.FlatStyle = "Flat"; $btnDamageRange.BackColor = $global:theme.ButtonBack; $btnDamageRange.ForeColor = $global:theme.TextMain; $btnDamageRange.FlatAppearance.BorderColor = $global:theme.OrangeAccent
    ...
    # [MODIFICADO] Adicionado o $btnDamageRange na lista de controles do painel e no hashtable $ui
    $pnlMenu.Controls.AddRange(@($btnManageItems, $btnCaliber, $btnCompat, $btnDamageRange, $btnBackMain))
    $ui.BtnManageItems = $btnManageItems; $ui.BtnCaliber = $btnCaliber; $ui.BtnCompat = $btnCompat; $ui.BtnDamageRange = $btnDamageRange; $ui.BtnBackMain = $btnBackMain
    ...
    foreach ($cat in $categories) {
        ...
        if ($cat.Name -eq "Armas") {
            ...
            $clickDel = {
                ...
                if ($sel) {
                    if ([System.Windows.Forms.MessageBox]::Show("Tem certeza absoluta que deseja APAGAR '$sel'?`nEssa ação é irreversível e excluirá TODOS os perfis de dano desta arma.", "Confirmar Exclusão", "YesNo", "Warning") -eq "Yes") {
                        # 1. Apaga a Arma do Weapons.csv
                        $newList = $allWeps | Where-Object { $_.NomeItem -ne $sel }
                        $newList | Export-Csv -Path $csvPath -NoTypeInformation -Delimiter ";" -Encoding UTF8
                        
                        # 2. [CASCATA DE SEGURANÇA] Apaga a Arma do DamageRange.csv
                        $dmgCsvPath = Join-Path (Get-DatabasePath) "DamageRange.csv"
                        if (Test-Path $dmgCsvPath) {
                            $dmgData = @(Import-Csv $dmgCsvPath -Delimiter ";" -Encoding UTF8)
                            $newDmgData = $dmgData | Where-Object { $_.Arma -ne $sel }
                            
                            # Exporta apenas se alguma linha foi realmente apagada
                            if ($newDmgData.Count -ne $dmgData.Count) {
                                $newDmgData | Export-Csv -Path $dmgCsvPath -NoTypeInformation -Delimiter ";" -Encoding UTF8
                            }
                        }
            ...
            $clickDel = {
                ...
                if ($selString) {
                    if ($selString -match '^(.*) \[(.*)\]$') {
                        $selName = $Matches[1]
                        $selCal  = $Matches[2]
                        if ([System.Windows.Forms.MessageBox]::Show("Tem certeza absoluta que deseja APAGAR '$selName'?`nEssa ação é irreversível e excluirá TODOS os perfis de dano associados a esta munição específica.", "Confirmar Exclusão", "YesNo", "Warning") -eq "Yes") {
                            # 1. Apaga a Munição do Ammo.csv
                            $newList = $allAmmo | Where-Object { -not ($_.NomeItem -eq $selName -and $_.Calibre -eq $selCal) }
                            $newList | Export-Csv -Path $csvPath -NoTypeInformation -Delimiter ";" -Encoding UTF8
                            
                            # 2. [CASCATA DE SEGURANÇA] Apaga a Munição do DamageRange.csv (Cruzamento Duplo)
                            $dmgCsvPath = Join-Path (Get-DatabasePath) "DamageRange.csv"
                            if (Test-Path $dmgCsvPath) {
                                $dmgData = @(Import-Csv $dmgCsvPath -Delimiter ";" -Encoding UTF8)
                                $newDmgData = $dmgData | Where-Object { -not ($_."nome da munição" -eq $selName -and $_.Calibre -eq $selCal) }
...
function Register-ManagerEvents {
    ...
    # Agora o botão abre diretamente o Painel Principal de Dano
    if ($ui.BtnDamageRange) {
        $ui.BtnDamageRange.Add_Click({
            Build-DamageRangePanels -ui $ui
            $ui.PnlMenu.Visible = $false
            $ui["PnlDamageMain"].Visible = $true
        }.GetNewClosure())
    }
        ...
        $escEvent = { 
            if ($_.KeyCode -eq "Escape") {
                if ($ui.PnlManagerRoot -and -not $ui.PnlManagerRoot.IsDisposed -and $ui.PnlManagerRoot.Visible) {
                    ...
                    # Tecla ESC sai do painel único de dano
                    elseif ($ui["PnlDamageMain"] -and $ui["PnlDamageMain"].Visible) {
                        $_.SuppressKeyPress = $true
                        $ui["PnlDamageMain"].Visible = $false
                        $ui.PnlMenu.Visible = $true
                    }
